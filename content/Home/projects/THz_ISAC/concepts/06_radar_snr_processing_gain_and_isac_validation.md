...
while sweeping the OMT isolation.  The default SI-on isolation is `25 dB`, and
...
- SI on, with the user-entered OMT isolation, default `27 dB`
...
The maximum-distance-vs-rho plot uses the same ZBD target-power law: the
displayed radar SINR trend follows approximately `1/R^4`, so the radar maximum
distance is solved with a `40 dB/decade` range exponent rather than the older
`20 dB/decade` amplitude-law approximation.
...
2. Set **SI-on isolation [dB]** and **SI-off isolation [dB]**.  The defaults are
   `25 dB` and `1000 dB` before synchronization.
...
if those points should be shown.
