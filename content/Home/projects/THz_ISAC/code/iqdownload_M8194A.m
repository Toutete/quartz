...
% remove the slave(s)
    ...
    if (isfield(arbConfig,'do_rst') && arbConfig.do_rst && ~isfield(arbConfig, 'setupSlave'))
        if (fs > 0 && sum(sum(abs(chMap))) > 0 && (isempty(find(chMap(:,1), 1)) || isempty(find(chMap(:,2), 1))))
    ...
    if (run > 0 && fs ~= 0 && ~isfield(arbConfig, 'setupSlave'))
        ...
        if (isfield(arbConfig, 'clockSource'))
            switch (arbConfig.clockSource)
                ...
                case 'ExtRef'
                    cmd = sprintf(':ROSC:SOURce EXT; :ROSC:EXT:FREQuency %.15g; ', arbConfig.clockFreq);
                case 'ExtClk'
    ...
    if (fs > 0)
        ...
        for col = 1:min(size(chMap, 2) / 2, size(data,2))
            for ch = find(chMap(:, 2*col-1))'
                gen_arb_M8194A(arbConfig, f, ch, real(data(:,col)), marker1, segmNum, run, fs);
            end
            for ch = find(chMap(:, 2*col))'
                gen_arb_M8194A(arbConfig, f, ch, imag(data(:,col)), marker2, segmNum, run, fs);
            end
        end
        if (fs > 0 && run > 0)
...
function gen_arb_M8194A(arbConfig, f, chan, data, marker, segm_num, run, fs)
% download an arbitrary waveform signal to a given channel and segment
    if (isempty(chan) || ~chan)
        return;
    end
    % apply channel delay (for historical reasons, the parameter is called skew
    if (isfield(arbConfig, 'skew'))
        tmp = fixlength(arbConfig.skew, chan);
        if (tmp(chan) ~= 0)
            data = iqdelay(data, fs, tmp(chan));
            % make sure we are still in the range [-1...1]
            scale = max(abs(data));
            if (scale > 1)
                data = data / scale;
            end
    ...
    if (isfield(arbConfig, 'peaking'))
        ...
        % note: iqengprintf will convert +Inf to "max" and -Inf to "min"
        xfprintf(f, sprintf(':OUTP%d:VPCORR %s', chan, iqengprintf(a(chan))));
    end
