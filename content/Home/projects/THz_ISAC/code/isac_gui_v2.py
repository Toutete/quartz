...
def calc_isac_link_budget(
    ...
    # Gamma is referenced to the antenna's conjugate match. The fraction
    # accepted by the communication receiver is therefore 1-|Gamma|^2. This
    # same Gamma drives antenna-mode reradiation below. Structural scattering
    # is retained separately and does not vanish at a matched load.
    comm_accepted_fraction = max(1.0 - gamma_mag ** 2, 0.0)
    comm_mismatch_loss_db = -10.0 * np.log10(max(comm_accepted_fraction, 1e-30))
    # Structural RCS is already an area in m^2 and remains when the antenna is
    # conjugately matched. Antenna mismatch and antenna polarization factors
    # therefore apply only to the antenna-mode reradiation term.
    sigma_struct_eff = sigma_struct
    ...
    c1_free_space_loss_db = fspl_one_way_db - tx_gain_dbi - rx_gain_dbi + 2.0 * omt_il_db
    c1_total_loss_db = c1_free_space_loss_db + comm_mismatch_loss_db
    ...
    return {
        ...
        "c1_free_space_loss_db": c1_free_space_loss_db,
        "target_gamma_mag": gamma_mag,
        "comm_accepted_fraction": comm_accepted_fraction,
        "comm_mismatch_loss_db": comm_mismatch_loss_db,
        ...
        "rcs_incoherent_sum_assumed": 1.0 - rcs_is_direct,
        "c1_rf_dbm": c1_rf_dbm,
        "c2_rf_dbm": c2_rf_dbm,
...
def run_isac_sim(cfg: SimConfig):
    ...
    sensing_ref_if_real = None
    if sensing_ref_bb is not None:
        sensing_ref_if_real = np.real(
            sensing_ref_bb * np.exp(1j * 2 * np.pi * f_if * t)
        )
        sensing_ref_if_real = apply_awg_dac_quantization(
            sensing_ref_if_real, cfg.awg_dac_bits
        )
    ...
    if cfg.rx_mode == 'ZBD':
        ...
        ref_wave = (
            sensing_ref_if_real
            if waveform_kind == "DFT-s-OFDM" and sensing_ref_if_real is not None
            else x_if_real
        )
        ref_sig = ref_wave - np.mean(ref_wave)
    ...
    c2_sensing_noise_dbm = float(
        c2_band_metrics.get(
            ...
            c2_band_metrics.get("noise_power_dbm", float("nan")),
        )
    )
    raw_radar_snr_db = (
        c2_target_power_dbm
        - c2_sensing_noise_dbm
        + pilot_weighted_proc_gain_db
        if np.isfinite(c2_target_power_dbm) and np.isfinite(c2_sensing_noise_dbm)
        else float("nan")
    )
    if np.isfinite(raw_radar_snr_db):
        raw_radar_snr = 10.0 ** (raw_radar_snr_db / 10.0)
        ...
        radar_snr_db = 10.0 * np.log10(
            ...
            / (
                ...
                + 1.0 / max(residual_ceiling, 1e-300)
            )
        )
    else:
    ...
    return {
        ...
        "target_rcs_mode": str(cfg.target_rcs_mode),
        "target_gamma_mag": link["target_gamma_mag"],
        "comm_accepted_fraction": link["comm_accepted_fraction"],
        "comm_mismatch_loss_db": link["comm_mismatch_loss_db"],
        ...
        "c2_spectrum_floor_snr_db": c2_band_metrics.get("snr_db", float("nan")),
        "snr_rad_post_db_c2": radar_snr_db,
        "range_profile_contrast_db_c2": range_profile_contrast_db,
        "radar_processing_gain_db_c2": proc_gain_db,
        "radar_pilot_weighted_gain_db_c2": pilot_weighted_proc_gain_db,
        "snr_rad_pg_corrected_db_c2": (
            c2_target_power_dbm - c2_sensing_noise_dbm
            if np.isfinite(c2_target_power_dbm)
            and np.isfinite(c2_sensing_noise_dbm)
            else float("nan")
        ),
...
class PhotonicIsacSimPanel:
    ...
    def _apply_sim_preset(self, preset: dict) -> None:
        ...
        if "target_rcs_mode" not in preset_params:
            # Legacy presets used a direct effective-RCS override and assumed
            # full communication absorption. Preserve that old behavior
            # explicitly instead of interpreting a legacy |Gamma|=1 as zero
            # communication power under the new coupled model.
            self.params["target_rcs_mode"].set("Direct effective RCS")
            self.params["target_gamma_mag"].set("0.0")
    ...
    def _build_ui(self):
        ...
        ttk.Combobox(
            grp,
            textvariable=self.params["target_rcs_mode"],
            values=["Coupled antenna", "Direct effective RCS"],
            state="readonly",
            width=18,
    ...
    def _update_table(self, *args):
        try:
            ...
            link = calc_isac_link_budget(
                ...
                tx_gain_dbi=tx_gain,
                rx_gain_dbi=rx_gain,
                rcs_sqm=self._param_float("target_rcs_sqm", 0.01),
            ...
            if "comm_mismatch" in self.rows:
                self.table.item(self.rows["comm_mismatch"], values=(f"{link['comm_mismatch_loss_db']:.2f}", "dB"))
            if "comm_accepted" in self.rows:
                self.table.item(self.rows["comm_accepted"], values=(f"{link['comm_accepted_fraction']:.4f}", ""))
    ...
    def _cfg_from_ui(self) -> SimConfig:
        ...
        cfg = SimConfig(
            ...
            target_effective_rcs_dbsm=effective_rcs_override,
    ...
    def _init_plot(self):
        ...
        for i, ax in enumerate(self.axes):
            ...
            if i == 2:
                ...
                self._spectrum_power_texts["C2"] = ax.text(
                    0.02, 0.04, "", transform=ax.transAxes, ha="left", va="bottom",
            ...
            if i == 3:
                ...
                self._spectrum_power_texts["C1"] = ax.text(
                    0.02, 0.04, "", transform=ax.transAxes, ha="left", va="bottom",
                    fontsize=7.5, color="#111827",
                    bbox={"boxstyle": "round,pad=0.25", "facecolor": "white", "edgecolor": "#94a3b8", "alpha": 0.88},
                )
    ...
    def run_simulation(self) -> None:
        try:
            ...
            c2rawm = self.data.get("c2_raw_band_metrics", {})
            ...
            if "c2_band" in self.rows:
                c2_is_phase_avg = c2m.get("metric_mode") == "phase_averaged_target"
                self.table.item(
                    self.rows["c2_band"],
                    text=(
                        "C2 Target Band Power (phase avg.)"
                        if c2_is_phase_avg
                        else "C2 Coherent Band Power (noise-sub.)"
                    ),
                )
                self.table.item(self.rows["c2_band"], values=(f"{float(c2m.get('band_power_dbm', np.nan)):.2f}", "dBm"))

            def _metric_dbm(metrics: dict, key: str) -> str:
                value = float(metrics.get(key, float("nan")))
                return f"{value:.2f}" if np.isfinite(value) else "N/A"

            c1_text = (
                f"Raw band: {_metric_dbm(c1m, 'raw_band_power_dbm')} dBm\n"
                f"Signal (noise-sub.): {_metric_dbm(c1m, 'band_power_dbm')} dBm"
            )
            c2_metric_label = (
                "Target (phase-avg.)"
                if c2m.get("metric_mode") == "phase_averaged_target"
                else "Coherent (noise-sub.)"
            )
            c2_text = (
                f"Raw band: {_metric_dbm(c2rawm, 'raw_band_power_dbm')} dBm\n"
                f"{c2_metric_label}: {_metric_dbm(c2m, 'band_power_dbm')} dBm"
            )
            if "C1" in self._spectrum_power_texts:
                self._spectrum_power_texts["C1"].set_text(c1_text)
            if "C2" in self._spectrum_power_texts:
                self._spectrum_power_texts["C2"].set_text(c2_text)
            ...
            if "range_contrast" in self.rows:
                self.table.item(
                    ...
                    values=(f"{float(self.data.get('range_profile_contrast_db', np.nan)):.2f}", "dB"),
                )
            ...
            if "pilot_gain" in self.rows:
                self.table.item(
                    self.rows["pilot_gain"],
                    values=(f"{float(self.data.get('pilot_weighted_processing_gain_db', np.nan)):.2f}", "dB"),
                )
...
class DsoPanel:
    ...
    def __init__(self, parent: ttk.Frame, runtime: dict) -> None:
        ...
        self._last_repeat_ensemble: dict[str, object] = {}
        self._last_loaded_capture_path: str | None = None
        self._measurement_records: list[dict[str, object]] = []
        self._measurement_raw: dict[str, dict[str, dict[str, object]]] = {}
        self._measurement_results: dict[str, object] = {}
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(conn_btn_bottom, text="Repeat Stats x10",
                   command=self._on_best_evm_acquire).pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(conn_btn_bottom, text="Power Stats x10",
        ...
        ttk.Button(btn_f_top, text="Range Stats x10",
        ...
        ttk.Entry(
            grp2, textvariable=self.radar_proc_gain_eff_var, width=10
        ...
        # Section 3: repeatable measurement campaign
        grp_campaign = ttk.LabelFrame(lf, text="Measurement Campaign", padding=8)
        grp_campaign.pack(fill=tk.X, pady=(0, 6))
        for col in range(4):
            grp_campaign.columnconfigure(col, weight=1 if col in {1, 3} else 0)

        self.measurement_set_name_var = tk.StringVar(
            value=f"remeasurement_{datetime.now().strftime('%Y%m%d')}"
        )
        self.measurement_range_mm_var = tk.StringVar(value="1000")
        self.measurement_iph_var = tk.StringVar(value="6.5")
        self.measurement_iph_unit_var = tk.StringVar(value="mA")
        self.measurement_clock_sync_var = tk.StringVar(value="Shared clock")
        self.measurement_keep_raw_var = tk.BooleanVar(value=False)
        self.measurement_direct_vpp_var = tk.StringVar(value="0.10, 0.20, 0.317")
        self.measurement_dso_scales_var = tk.StringVar(value="100/20, 200/50")
        self.measurement_np_list_var = tk.StringVar(value="128, 256, 512, 1024")
        self.measurement_rho_list_var = tk.StringVar(value="0.10, 0.20, 0.40")
        self.measurement_sweep_state_var = tk.StringVar(value="Target-on")
        self.measurement_tx_power_dbm_var = tk.StringVar(value="-8")
        self.measurement_ant_gain_var = tk.StringVar(value="25, 25")
        self.measurement_rx_gain_db_var = tk.StringVar(value="0")
        self.measurement_system_loss_db_var = tk.StringVar(value="0")
        self.measurement_status_var = tk.StringVar(value="Set: 0 records")

        ttk.Label(grp_campaign, text="Set name").grid(row=0, column=0, sticky="w", pady=2)
        ttk.Entry(
            grp_campaign, textvariable=self.measurement_set_name_var, width=18
        ).grid(row=0, column=1, sticky="ew", pady=2)
        ttk.Label(grp_campaign, text="Range (mm)").grid(
            row=0, column=2, sticky="w", padx=(8, 0), pady=2
        )
        ttk.Entry(
            grp_campaign, textvariable=self.measurement_range_mm_var, width=9
        ).grid(row=0, column=3, sticky="ew", pady=2)

        ttk.Label(grp_campaign, text="Photocurrent").grid(row=1, column=0, sticky="w", pady=2)
        iph_frame = ttk.Frame(grp_campaign)
        iph_frame.grid(row=1, column=1, sticky="ew", pady=2)
        ttk.Entry(iph_frame, textvariable=self.measurement_iph_var, width=10).pack(
            side=tk.LEFT, fill=tk.X, expand=True
        )
        ttk.Combobox(
            iph_frame,
            textvariable=self.measurement_iph_unit_var,
            values=["nA", "uA", "mA"],
            state="readonly",
            width=4,
        ).pack(side=tk.LEFT, padx=(4, 0))
        ttk.Label(grp_campaign, text="Clock").grid(
            row=1, column=2, sticky="w", padx=(8, 0), pady=2
        )
        ttk.Combobox(
            grp_campaign,
            textvariable=self.measurement_clock_sync_var,
            values=["Shared clock", "Free running"],
            state="readonly",
            width=13,
        ).grid(row=1, column=3, sticky="ew", pady=2)

        direct_frame = ttk.Frame(grp_campaign)
        direct_frame.grid(row=2, column=0, columnspan=4, sticky="ew", pady=(5, 0))
        ttk.Label(direct_frame, text="Vpp sweep").pack(side=tk.LEFT)
        ttk.Entry(
            direct_frame, textvariable=self.measurement_direct_vpp_var, width=15
        ).pack(side=tk.LEFT, padx=(4, 8))
        ttk.Label(direct_frame, text="C1/C2 mV/div").pack(side=tk.LEFT)
        ttk.Entry(
            direct_frame, textvariable=self.measurement_dso_scales_var, width=13
        ).pack(side=tk.LEFT, padx=(4, 0), fill=tk.X, expand=True)

        direct_btns = ttk.Frame(grp_campaign)
        direct_btns.grid(row=3, column=0, columnspan=4, sticky="w", pady=(4, 0))
        ttk.Button(
            direct_btns, text="AWG-DSO Direct", command=self._on_measure_direct_point
        ).pack(side=tk.LEFT)
        ttk.Button(
            direct_btns, text="Direct Sweep", command=self._on_measure_direct_sweep
        ).pack(side=tk.LEFT, padx=(5, 0))
        ttk.Checkbutton(
            direct_btns, text="Keep raw", variable=self.measurement_keep_raw_var
        ).pack(side=tk.LEFT, padx=(8, 0))

        pilot_frame = ttk.Frame(grp_campaign)
        pilot_frame.grid(row=4, column=0, columnspan=4, sticky="ew", pady=(5, 0))
        ttk.Label(pilot_frame, text="Np").pack(side=tk.LEFT)
        ttk.Entry(
            pilot_frame, textvariable=self.measurement_np_list_var, width=17
        ).pack(side=tk.LEFT, padx=(4, 8))
        ttk.Label(pilot_frame, text="rho").pack(side=tk.LEFT)
        ttk.Entry(
            pilot_frame, textvariable=self.measurement_rho_list_var, width=12
        ).pack(side=tk.LEFT, padx=(4, 8))
        ttk.Combobox(
            pilot_frame,
            textvariable=self.measurement_sweep_state_var,
            values=["SI-only", "Target-on"],
            state="readonly",
            width=10,
        ).pack(side=tk.LEFT)

        pilot_btns = ttk.Frame(grp_campaign)
        pilot_btns.grid(row=5, column=0, columnspan=4, sticky="w", pady=(4, 0))
        ttk.Button(
            pilot_btns, text="Pilot Point", command=self._on_measure_pilot_point
        ).pack(side=tk.LEFT)
        ttk.Button(
            pilot_btns, text="Np/rho Sweep", command=self._on_measure_pilot_sweep
        ).pack(side=tk.LEFT, padx=(5, 0))
        ttk.Button(
            pilot_btns,
            text="Estimate Gp",
            command=lambda: self._show_campaign_calculation("gp"),
        ).pack(side=tk.LEFT, padx=(5, 0))

        state_btns = ttk.Frame(grp_campaign)
        state_btns.grid(row=6, column=0, columnspan=4, sticky="ew", pady=(6, 0))
        state_specs = [
            ("1 RX Noise", "receiver_noise"),
            ("2 Carrier Noise", "carrier_noise"),
            ("3 SI-only", "si_only"),
            ("4 Target-on", "target_on"),
        ]
        for index, (label, condition) in enumerate(state_specs):
            ttk.Button(
                state_btns,
                text=label,
                command=lambda c=condition: self._on_campaign_measure(c),
            ).grid(
                row=index // 2,
                column=index % 2,
                sticky="ew",
        ...
        state_btns.columnconfigure(0, weight=1)
        state_btns.columnconfigure(1, weight=1)

        rcs_frame = ttk.Frame(grp_campaign)
        rcs_frame.grid(row=7, column=0, columnspan=4, sticky="ew", pady=(6, 0))
        ttk.Label(rcs_frame, text="TX dBm").pack(side=tk.LEFT)
        ttk.Entry(
            rcs_frame, textvariable=self.measurement_tx_power_dbm_var, width=7
        ).pack(side=tk.LEFT, padx=(3, 7))
        ttk.Label(rcs_frame, text="Gt,Gr dBi").pack(side=tk.LEFT)
        ttk.Entry(
            rcs_frame, textvariable=self.measurement_ant_gain_var, width=9
        ).pack(side=tk.LEFT, padx=(3, 7))
        rcs_cal_frame = ttk.Frame(grp_campaign)
        rcs_cal_frame.grid(row=8, column=0, columnspan=4, sticky="ew", pady=(3, 0))
        ttk.Label(rcs_cal_frame, text="RX cal dB").pack(side=tk.LEFT)
        ttk.Entry(
            rcs_cal_frame, textvariable=self.measurement_rx_gain_db_var, width=6
        ).pack(side=tk.LEFT, padx=(3, 7))
        ttk.Label(rcs_cal_frame, text="Loss dB").pack(side=tk.LEFT)
        ttk.Entry(
            rcs_cal_frame, textvariable=self.measurement_system_loss_db_var, width=6
        ).pack(side=tk.LEFT, padx=(3, 0))

        calc_btns = ttk.Frame(grp_campaign)
        calc_btns.grid(row=9, column=0, columnspan=4, sticky="w", pady=(5, 0))
        ttk.Button(
            calc_btns,
            text="Calculate SNR",
            command=lambda: self._show_campaign_calculation("snr"),
        ).pack(side=tk.LEFT)
        ttk.Button(
            calc_btns,
            text="Estimate RCS",
            command=lambda: self._show_campaign_calculation("rcs"),
        ).pack(side=tk.LEFT, padx=(5, 0))
        file_btns = ttk.Frame(grp_campaign)
        file_btns.grid(row=10, column=0, columnspan=4, sticky="w", pady=(4, 0))
        ttk.Button(
            file_btns, text="Save Set", command=self._on_save_measurement_set
        ).pack(side=tk.LEFT, padx=(5, 0))
        ttk.Button(
            file_btns, text="Load Set", command=self._on_load_measurement_set
        ).pack(side=tk.LEFT, padx=(5, 0))
        ttk.Button(
            file_btns, text="Clear", command=self._on_clear_measurement_set
        ).pack(side=tk.LEFT, padx=(5, 0))
        ttk.Label(
            grp_campaign,
    ...
    def _snapshot_capture_state(self, compact: bool = False) -> dict:
        sample_dtype = np.float32 if compact else np.float64
        rx_multi = {}
        for ch, item in (self._rx_multi or {}).items():
            rx_multi[ch] = {
                "sig": np.asarray(item["sig"], dtype=sample_dtype).copy(),
                "t": np.asarray(item["t"], dtype=sample_dtype).copy(),
                "fs": float(item["fs"]),
            }
        return {
            "rx_multi": rx_multi,
            ...
            "rx_sig": None if self._rx_sig is None else np.asarray(self._rx_sig, dtype=sample_dtype).copy(),
            "rx_t": None if self._rx_t is None else np.asarray(self._rx_t, dtype=sample_dtype).copy(),
        ...
        if extra:
            for key, value in extra.items():
                if isinstance(value, (bool, np.bool_)):
                    ...
                    value_f = float(value)
                    record[key] = value_f if np.isfinite(value_f) else None
                elif value is not None:
                    record[key] = str(value)
        return record

    @staticmethod
    def _repeat_numeric_stats(
    ...
    @classmethod
    def _representative_repeat_index(
        cls,
        records: list[dict[str, object]],
        ...
        if not records:
            raise ValueError("No repeat records are available.")
        candidates = list(valid_indices or range(len(records)))
        if not candidates:
        ...
        for key in metric_keys:
            ...
            for i in candidates:
                try:
                    values_list.append(float(records[i].get(key, float("nan"))))
                except Exception:
                    values_list.append(float("nan"))
            ...
            scale = (
                ...
                else max(float(np.std(finite)), 1e-9)
            )
            usable.append((key, center, scale))
        if not usable:
            return int(candidates[len(candidates) // 2])
        scored: list[tuple[float, int]] = []
        for i in candidates:
            terms = []
            for key, center, scale in usable:
                try:
                    value = float(records[i].get(key, float("nan")))
                except Exception:
                    continue
                if np.isfinite(value):
                    terms.append(min(abs(value - center) / scale, 6.0) ** 2)
            scored.append((float(np.mean(terms)) if terms else float("inf"), i))
        return int(min(scored)[1])

    def _store_repeat_ensemble(
        self,
        mode: str,
        records: list[dict[str, object]],
        selected_index: int,
    ) -> None:
        stats = self._repeat_numeric_stats(records)
        self._last_repeat_ensemble = {
            "schema": "isac-repeat-ensemble-v2",
            "mode": str(mode),
            "selected_index": int(selected_index),
            "records": copy.deepcopy(records),
            "stats": stats,
        }
        self._set_metric("repeat_count", "Repeat Count", len(records), "")
        self._set_metric(
            "repeat_selected_index",
            "Representative Run",
            int(selected_index) + 1,
            "",
            "Run nearest the robust multimetric median; raw waveform retained for this run only.",
        )
        for key in ("evm_db", "evm_snr", "snr_rad_db", "range_profile_snr_db", "total_band_power_dbm"):
            item = stats.get(key)
            if not item:
                continue
            self._set_metric(
                f"repeat_{key}_median",
                f"Repeat median: {key}",
                item["median"],
                "dB"
                if key == "evm_snr"
                or key.endswith("_db")
                or key.endswith("_dbm")
                else "",
                f"IQR={item['p75'] - item['p25']:.3g}, MAD={item['mad']:.3g}, n={int(item['count'])}",
                "Repeatability",
            )
    ...
    def _update_band_metrics_for_channels(self) -> dict[str, dict[str, float]]:
        ...
        for row, ch in enumerate(self._display_dso_channels()):
            ...
            if row == 1:
                self._set_metric(
                    "c2_total_band_power_dbm",
                    "C2 Total Band Power",
                    vals["raw_power_dbm"],
                    "dBm",
                    f"Raw C2 in-band power from monostatic channel {ch}; includes SI and background.",
                )
                self._set_metric(
                    "c2_total_band_snr_db",
                    "C2 Total Band SNR",
                    vals["snr_db"],
                    "dB",
                    f"Diagnostic total-band SNR from monostatic channel {ch}; not target-only when SI is present.",
                )
        return out
    ...
    def _clear_demod_metrics(self) -> None:
        for key in list(self._metrics.keys()):
            if (
                ...
                or key.startswith("evm_db_")
                or key.startswith("evm_pct_")
                or key.startswith("evm_snr_")
                or key.startswith("evm_snr_db_")
                or key.startswith("sinr_com_db_")
                or key.startswith("ber_")
                or key.startswith("symbols_")
            ):
    ...
    def _clear_range_metrics(self) -> None:
        for key in list(self._metrics.keys()):
            if key.startswith("range_peak_m") or key.startswith("range_peak_mm") or key.startswith("pslr_db") or key.startswith("diff_range_mm") or key.startswith("range_difference_mm") or key.startswith("cfr_"):
                ...
                key == "snr_rad_db"
                or key.startswith("snr_rad_post_db")
                or key.startswith("radar_processing_gain_db")
                or key.startswith("radar_ideal_processing_gain_db")
                or key.startswith("radar_pilot_weighted_gain_db")
                or key.startswith("radar_pre_snr_db")
                or key.startswith("range_profile_contrast_db")
                or key.startswith("range_floor_iqr_db")
                or key.startswith("snr_rad_pg_corrected_db")
            ):
                self._metrics.pop(key, None)
    ...
    def _on_best_power_acquire(self) -> None:
        def worker():
            ...
            try:
                for idx in range(10):
                    self._log(f"[Power Stats] Acquire {idx + 1}/10")
                    ...
                    total_dbm = 10.0 * np.log10(max(score, 1e-30))
                    desc = ", ".join(
                    ...
                    extra: dict[str, object] = {
                        "total_band_power_dbm": total_dbm,
                    }
                    for ch, value in vals.items():
                        extra[f"{ch.lower()}_band_power_dbm"] = value.get(
                            "band_power_dbm", float("nan")
                        )
                        extra[f"{ch.lower()}_band_snr_db"] = value.get(
                            "snr_db", float("nan")
                        )
                    records.append(self._repeat_record(idx, extra))
                    snapshots.append(self._snapshot_capture_state(compact=True))
                    self._log(
                        f"[Power Stats] {idx + 1}/10 total={total_dbm:.2f} dBm  {desc}"
                    )
                if not snapshots:
                    raise ValueError("No valid capture was acquired.")
                selected = self._representative_repeat_index(
                    records,
                    (
                        "total_band_power_dbm",
                        "c1_band_power_dbm",
                        "c2_band_power_dbm",
                        "c1_band_snr_db",
                        "c2_band_snr_db",
                ...
                self._restore_capture_state(snapshots[selected])
                self._update_band_metrics_for_channels()
                self._store_repeat_ensemble("power-stats-x10", records, selected)
                stats = self._last_repeat_ensemble.get("stats", {})
                p_stats = stats.get("total_band_power_dbm", {}) if isinstance(stats, dict) else {}
                self._log(
                    f"[Power Stats] Representative run={selected + 1}/10, "
                    f"median={float(p_stats.get('median', float('nan'))):.2f} dBm, "
                    f"IQR={float(p_stats.get('p75', float('nan'))) - float(p_stats.get('p25', float('nan'))):.2f} dB."
                )
                self.parent.after(
                    0,
                    lambda s=selected: self.capture_file_var.set(
                        f"Capture: power-stats representative {s + 1}/10 (unsaved)"
                    ),
                )
                self.parent.after(0, self._plot_spectrum_and_time)
                self.parent.after(0, self._refresh_metrics_table)
            except Exception as e:
                self._log(f"[Power Stats] Error: {e}")
    ...
    def _on_best_evm_acquire(self) -> None:
        self._run_best_comm_range_acquire("Repeat Stats")
    ...
    def _run_best_range_only_acquire(self) -> None:
        def worker():
            ...
            try:
                ...
                min_quality_db = 12.0
                self._log(
                    f"[Range Stats] Range-only ensemble: channel={range_ch}, "
                ...
                for idx in range(10):
                    run_no = idx + 1
                    self._log(f"[Range Stats] Acquire {run_no}/10")
                    self.parent.after(
                        0,
                        lambda n=run_no: self.capture_file_var.set(
                            f"Capture: Range Stats {n}/10 acquiring..."
                        ),
                    )
                    self._capture_live_once_sync()
                    self._update_band_metrics_for_channels()
                    ...
                    self._log(
                        f"[Range Stats] {run_no}/10 range={est_range:.4g} m "
                    ...
                    self.parent.after(
                        ...
                        lambda n=run_no, r=est_range, d=diff_mm, p=pnr_db, s=sep_db: self.capture_file_var.set(
                            f"Capture: Range Stats {n}/10  range={r:.4g} m  dR={d:.2f} mm  PNR={p:.2f} dB  sep={s:.2f} dB"
                        ),
                    )
                    self.parent.after(0, lambda item=result: self._show_isac_range_results([item]))

                    records.append(self._repeat_record(idx, {
                        "range_m": est_range,
                        "range_diff_mm": diff_mm,
                        "range_diff_err_mm": diff_err_mm,
                        "range_profile_snr_db": float(result.get("range_profile_snr_db", float("nan"))),
                        "pslr_db": float(result.get("pslr_db", float("nan"))),
                        "peak_noise_db": pnr_db,
                        "peak_sep_db": sep_db,
                        "range_quality_ok": bool(summary.get("ok", False)),
                        "range_method": method,
                    }))
                    snapshots.append(self._snapshot_capture_state(compact=True))
                ...
                valid_indices = [
                    ...
                    if bool(record.get("range_quality_ok", False))
                ]
                selected = self._representative_repeat_index(
                    records,
                    (
                ...
                if item is None:
                    item = {"sig": self._rx_sig, "fs": self._rx_fs}
                representative_result = self._compute_isac_range_profile_for_signal(
                    np.asarray(item["sig"], dtype=np.float64),
                    float(item["fs"]),
                    pl,
                    ch_label=range_ch,
                ...
                self._store_repeat_ensemble("range-stats-x10", records, selected)
                self._log(
                    f"[Range Stats] Selected robust representative run {selected + 1}/10; "
                    "all scalar results retained in the ensemble metadata."
                )
                self.parent.after(
                    0,
                    lambda s=selected: self.capture_file_var.set(
                        f"Capture: range-stats representative {s + 1}/10 (unsaved)"
                    ),
                )
                self.parent.after(0, self._plot_spectrum_and_time)
                display_channels = self._display_dso_channels()
                ...
                self.parent.after(
                    0,
                    lambda item=representative_result: self._show_isac_range_results([item]),
                )
                self.parent.after(0, self._refresh_metrics_table)
            except Exception as e:
                self._log(f"[Range Stats] Error: {e}")
                self.parent.after(0, lambda m=str(e): messagebox.showerror("Range Stats Error", m))
            ...
            try:
                ...
                for idx in range(10):
                    ...
                    records.append(self._repeat_record(idx, {
                        ...
                        "range_diff_err_mm": range_err_mm,
                        "range_profile_snr_db": (
                            float(range_result.get("range_profile_snr_db", float("nan")))
                            if range_result else float("nan")
                        ),
                        "pslr_db": (
                            float(range_result.get("pslr_db", float("nan")))
                            if range_result else float("nan")
                        ),
                        "peak_noise_db": pnr_db,
                        "peak_sep_db": sep_db,
                        "range_quality_ok": bool(range_summary.get("ok", False)) if range_summary else False,
                ...
                valid_indices = [
                    i for i, record in enumerate(records)
                    if record.get("evm_db") is not None
                    and np.isfinite(float(record["evm_db"]))
                    and bool(record.get("range_quality_ok", False))
                ]
                ...
                self._restore_capture_state(snapshots[selected])
                self._update_band_metrics_for_channels()
                self._store_repeat_ensemble("comm-range-stats-x10", records, selected)
                self._log(
                    f"[{label}] Selected joint representative run {selected + 1}/10. "
                    "C1 and C2 remain from the same trigger."
                )
                self.parent.after(
                    0,
                    lambda s=selected, lab=label: self.capture_file_var.set(
                        f"Capture: {lab} representative {s + 1}/10 (unsaved)"
                    ),
                )
                self.parent.after(0, self._plot_spectrum_and_time)
                self.parent.after(0, self._refresh_metrics_table)
    ...
    def _json_safe(value):
        if isinstance(value, dict):
            return {str(k): DsoPanel._json_safe(v) for k, v in value.items()}
        if isinstance(value, (list, tuple)):
            return [DsoPanel._json_safe(v) for v in value]
        if isinstance(value, np.ndarray):
            return DsoPanel._json_safe(value.tolist())
    ...
    @staticmethod
    def _parse_number_list(raw: str, *, positive: bool = False) -> list[float]:
        values: list[float] = []
        for token in str(raw).replace(";", ",").split(","):
            ...
            if not np.isfinite(value) or (positive and value <= 0.0):
                raise ValueError(f"Invalid sweep value: {token}")
            values.append(float(value))
        if not values:
            raise ValueError("Enter at least one sweep value.")
        return values

    @staticmethod
    def _parse_scale_pairs(raw: str) -> list[tuple[float, float]]:
        ...
        for token in str(raw).replace(";", ",").split(","):
            ...
            if len(parts) == 1:
                first = second = float(parts[0])
            elif len(parts) == 2:
                first, second = float(parts[0]), float(parts[1])
            else:
                raise ValueError(f"Invalid C1/C2 scale pair: {token}")
            if first <= 0.0 or second <= 0.0:
                raise ValueError("DSO voltage scales must be positive.")
            pairs.append((first, second))
        if not pairs:
            raise ValueError("Enter at least one DSO scale or C1/C2 scale pair.")
        return pairs

    def _measurement_photocurrent(self) -> tuple[float, float, str]:
        value = float(self.measurement_iph_var.get())
        unit = self.measurement_iph_unit_var.get().strip()
        scale_ma = {"nA": 1e-6, "uA": 1e-3, "mA": 1.0}.get(unit)
        if scale_ma is None or not np.isfinite(value) or value < 0.0:
            raise ValueError("Invalid photocurrent value or unit.")
        ma = value * scale_ma
        return ma, ma * 1e6, unit

    def _measurement_payload_values(self) -> tuple[dict, int, float]:
        ...
        npilot = int(
            payload.get(
                "requested_n_sym_per_chirp",
                payload.get("dft_n_data", payload.get("n_sym_per_chirp", 0)),
            )
            or 0
        )
        try:
            rho = float(payload.get("amplitude_ratio_rho", float("nan")))
        except Exception:
            rho = float("nan")
        return payload, npilot, rho

    def _measurement_metric_snapshot(self) -> dict[str, dict[str, object]]:
        self._update_derived_metrics()
        return {
            key: {
                "value": self._json_safe(item.get("value")),
                "unit": str(item.get("unit", "")),
                "label": str(item.get("label", key)),
                "note": str(item.get("note", "")),
                "category": str(item.get("category", "")),
    ...
    @staticmethod
    def _measurement_metric_value(
        record: dict[str, object], key: str, default: float = float("nan")
    ) -> float:
        try:
            item = dict(record.get("metrics", {})).get(key, {})
            value = item.get("value", default) if isinstance(item, dict) else item
            return float(value)
        except Exception:
            return float(default)

    @staticmethod
    def _measurement_condition_label(condition: str) -> str:
        return {
            "receiver_noise": "UTC-PD dark / receiver noise",
            "carrier_noise": "Normal Iph / AWG IF off / carrier-only",
            "si_only": "Waveform on / target-off absorber / SI-only",
            "target_on": "Waveform on / target-on",
            "awg_dso_direct": "AWG-DSO direct",
    ...
    def _measurement_record(
        ...
        range_items: list[dict] | None = None,
        *,
        sweep_kind: str = "",
    ) -> dict[str, object]:
        payload, npilot, rho = self._measurement_payload_values()
        iph_ma, iph_na, iph_unit = self._measurement_photocurrent()
        self.photocurrent_ma_var.set(f"{iph_ma:.12g}")
        record_id = (
            f"{datetime.now().strftime('%Y%m%dT%H%M%S%f')}_"
            f"{len(self._measurement_records) + 1:04d}"
        )
        scalar_ranges: list[dict[str, object]] = []
        for item in range_items or []:
            scalar_ranges.append(
                {
                    key: self._json_safe(item.get(key))
                    for key in (
                        "ch",
                        "row",
                        "range_mode",
        ...
        try:
            tx_vpp_1 = float(payload.get("awg_ch1_vpp", float("nan")))
        except Exception:
            tx_vpp_1 = float("nan")
        try:
            tx_vpp_2 = float(payload.get("awg_ch2_vpp", float("nan")))
        except Exception:
            tx_vpp_2 = float("nan")
        try:
            rf_ghz = float(payload.get("fc", float("nan"))) / 1e9
        except Exception:
            rf_ghz = float("nan")
        record: dict[str, object] = {
            "record_id": record_id,
            "schema": "isac-measurement-record-v1",
            "created": datetime.now().isoformat(timespec="milliseconds"),
            "set_name": self.measurement_set_name_var.get().strip(),
            "condition": condition,
            "condition_label": self._measurement_condition_label(condition),
            "sweep_kind": sweep_kind,
            "range_mm": float(self.measurement_range_mm_var.get()),
            "photocurrent_value": float(self.measurement_iph_var.get()),
            "photocurrent_unit": iph_unit,
            "photocurrent_ma": iph_ma,
            "photocurrent_na": iph_na,
            "clock_sync": self.measurement_clock_sync_var.get().strip(),
            "pilot_np": npilot,
            "rho": self._json_safe(rho),
            "waveform_type": str(payload.get("waveform_type", "")),
            "modulation": str(payload.get("modulation", self.demod_mod_var.get())),
            "rf_ghz": self._json_safe(rf_ghz),
            "symbol_rate_ghz": self._json_safe(
                float(payload.get("symbol_rate", float("nan"))) / 1e9
                if payload else float("nan")
            ),
            "awg_vpp_ch1": self._json_safe(tx_vpp_1),
            "awg_vpp_ch2": self._json_safe(tx_vpp_2),
            "dso_scale_ch1_mv_div": float(self.ch1_scale_mv_var.get()),
            "dso_scale_ch2_mv_div": float(self.ch2_scale_mv_var.get()),
            "tx_output_dbm": float(self.measurement_tx_power_dbm_var.get()),
            "channels": self._json_safe(band_by_channel),
            "metrics": self._measurement_metric_snapshot(),
            "range_results": scalar_ranges,
            "repeat_ensemble": self._json_safe(self._last_repeat_ensemble),
        }
        if bool(self.measurement_keep_raw_var.get()):
            raw_by_ch: dict[str, dict[str, object]] = {}
            for ch, item in (self._rx_multi or {}).items():
                raw_by_ch[ch] = {
                    "sig": np.asarray(item["sig"], dtype=np.float32).copy(),
                    "fs": float(item["fs"]),
                }
            if not raw_by_ch and self._rx_sig is not None:
                raw_by_ch[self.ch_var.get().strip().upper() or "C1"] = {
                    "sig": np.asarray(self._rx_sig, dtype=np.float32).copy(),
                    "fs": float(self._rx_fs),
                }
            if raw_by_ch:
                self._measurement_raw[record_id] = raw_by_ch
                record["raw_included"] = True
        record.setdefault("raw_included", False)
        return record

    def _measurement_channel(
        self, record: dict[str, object], *, sensing: bool
    ) -> tuple[str, dict[str, float]] | tuple[None, None]:
        channels = record.get("channels", {})
        if not isinstance(channels, dict) or not channels:
            return None, None
        normalized = {str(key).strip().upper(): value for key, value in channels.items()}
        preferred = "C2" if sensing else "C1"
        if preferred in normalized:
            return preferred, normalized[preferred]
        keys = sorted(normalized)
        key = keys[min(1 if sensing else 0, len(keys) - 1)]
        return key, normalized[key]

    def _measurement_raw_power_mw(
        self, record: dict[str, object], *, sensing: bool
    ) -> float:
        _ch, values = self._measurement_channel(record, sensing=sensing)
        if not isinstance(values, dict):
            return float("nan")
        try:
            dbm = float(values.get("raw_power_dbm", float("nan")))
    ...
    def _matching_measurement(
        ...
        *,
        strict_waveform: bool = False,
    ) -> dict[str, object] | None:
        candidates = [
            record
            for record in self._measurement_records
        ...
        def score(record: dict[str, object]) -> float:
            value = 0.0
            if str(record.get("clock_sync", "")) != str(reference.get("clock_sync", "")):
                value += 1e6 if strict_waveform else 1000.0
            scale_1_delta = abs(
                float(record.get("dso_scale_ch1_mv_div", 0.0))
                - float(reference.get("dso_scale_ch1_mv_div", 0.0))
            )
            scale_2_delta = abs(
                float(record.get("dso_scale_ch2_mv_div", 0.0))
                - float(reference.get("dso_scale_ch2_mv_div", 0.0))
            )
            if strict_waveform and (scale_1_delta > 1e-9 or scale_2_delta > 1e-9):
                value += 1e6
            value += abs(
                float(record.get("dso_scale_ch1_mv_div", 0.0))
                - float(reference.get("dso_scale_ch1_mv_div", 0.0))
            ...
            if condition != "receiver_noise":
                range_delta = abs(
                    float(record.get("range_mm", 0.0))
                    - float(reference.get("range_mm", 0.0))
                )
                current_delta = abs(
                    float(record.get("photocurrent_ma", 0.0))
                    - float(reference.get("photocurrent_ma", 0.0))
                )
                if strict_waveform and (range_delta > 1e-6 or current_delta > 1e-9):
                    value += 1e6
                value += 10.0 * range_delta
                value += 100.0 * current_delta
            if strict_waveform:
                if int(record.get("pilot_np", 0)) != int(reference.get("pilot_np", 0)):
                    value += 1e6
                if not close(record.get("rho"), reference.get("rho"), 1e-9):
                    value += 1e6
                if str(record.get("waveform_type", "")) != str(reference.get("waveform_type", "")):
                    value += 1e6
                if not close(record.get("awg_vpp_ch1"), reference.get("awg_vpp_ch1"), 1e-9):
                    value += 1e6
                if not close(record.get("awg_vpp_ch2"), reference.get("awg_vpp_ch2"), 1e-9):
                    value += 1e6
            return value

        best = min(candidates, key=score)
        if strict_waveform and score(best) >= 1e6:
            return None
        return best

    def _store_receiver_noise_reference(
        self, band_by_channel: dict[str, dict[str, float]]
    ) -> None:
        channels = self._display_dso_channels()
        if not channels:
            return
        values = band_by_channel.get(channels[0])
        if not isinstance(values, dict):
            return
        raw_mw = 10.0 ** (float(values["raw_power_dbm"]) / 10.0)
        bw_hz = max(float(values["analysis_bw_hz"]), 1.0)
        self._noise_floor_ref_dbmhz = 10.0 * np.log10(max(raw_mw / bw_hz, 1e-300))
        self._set_metric(
            "noise_floor_dbmhz",
            "Noise Density",
            self._noise_floor_ref_dbmhz,
            "dBm/Hz",
            "In-band UTC-PD-dark receiver-noise measurement from the current campaign.",
        )

    def _append_measurement_record(self, record: dict[str, object]) -> None:
        self._measurement_records.append(record)
        self.runtime["measurement_set_records"] = self._measurement_records
        self._autosave_measurement_set()
        self.parent.after(0, self._update_measurement_status)

    def _update_measurement_status(self, suffix: str = "") -> None:
        counts: dict[str, int] = {}
        for record in self._measurement_records:
            key = str(record.get("condition", "unknown"))
            counts[key] = counts.get(key, 0) + 1
        compact = ", ".join(
            f"{key}={counts[key]}"
            for key in ("receiver_noise", "carrier_noise", "si_only", "target_on", "awg_dso_direct")
            if counts.get(key)
        )
        text = f"Set: {len(self._measurement_records)} records"
        if compact:
            text += f" ({compact})"
        if suffix:
            text += f" | {suffix}"
        self.measurement_status_var.set(text)

    def _prepare_campaign_condition_sync(self, condition: str) -> None:
        ...
        if source is None:
            return
        address = (
            f"TCPIP0::{source.ip_var.get().strip()}::"
            f"{int(source.port_var.get())}::SOCKET"
        )
        if condition in {"receiver_noise", "carrier_noise"}:
            stop_awg(awg_addr=address, channels=[1, 2, 3, 4])
        elif condition in {"si_only", "target_on", "awg_dso_direct"}:
            channels = parse_channels(source.ch_var.get()) or [1, 2]
            run_awg(
                awg_addr=address,
                channels=channels,
                vpp=source._vpp_by_channel(channels),
            )
        time.sleep(0.25)

    def _campaign_capture_sync(
        self, condition: str, *, sweep_kind: str = ""
    ) -> dict[str, object]:
        self._prepare_campaign_condition_sync(condition)
        if bool(self.live_var.get()):
            self._capture_live_once_sync()
        elif self._rx_sig is None:
            raise ValueError("No capture in memory. Enable Live DSO or acquire/load data first.")
        self._clear_demod_metrics()
        self._clear_range_metrics()
        band_by_channel = self._update_band_metrics_for_channels()
        if condition == "receiver_noise":
            self._store_receiver_noise_reference(band_by_channel)

        payload = self._load_tx_payload_for_isac()
        range_items: list[dict] = []
        if payload is not None and condition in {"si_only", "target_on"}:
            try:
                range_items = self._compute_range_items_for_current_capture(payload)
                if range_items:
                    self.parent.after(
                        0,
                        lambda items=range_items: self._show_isac_range_results(items),
                    )
            except Exception as exc:
                self._log(f"[Set] Range processing skipped: {exc}")
        if condition in {"awg_dso_direct", "target_on"}:
            try:
                self._on_demodulate(show_errors=False, run_async=False)
                self._wait_for_metric("evm_db", timeout_s=0.8)
            except Exception as exc:
                self._log(f"[Set] Demodulation skipped: {exc}")

        record = self._measurement_record(
            condition, band_by_channel, range_items, sweep_kind=sweep_kind
        )
        self._append_measurement_record(record)
        if condition == "target_on":
            try:
                self._on_calculate_campaign_snr(show_dialog=False)
            except Exception as exc:
                self._log(f"[Set] Derived SNR pending: {exc}")
            try:
                self._on_estimate_processing_gain(show_dialog=False)
            except Exception as exc:
                self._log(f"[Set] Processing-gain estimate pending: {exc}")
        self._log(
            f"[Set] Recorded {record['condition_label']}: "
            f"R={record['range_mm']:.3f} mm, Iph={record['photocurrent_value']:g} "
            f"{record['photocurrent_unit']}, Np={record['pilot_np']}, rho={record['rho']}"
        )
        self.parent.after(0, self._plot_spectrum_and_time)
        self.parent.after(0, self._refresh_metrics_table)
        return record

    def _start_measurement_worker(self, label: str, action) -> None:
        if self._measurement_busy:
            messagebox.showwarning("Measurement Busy", "Another measurement is still running.")
            return
        self._measurement_busy = True

        def worker():
            try:
                self.parent.after(0, lambda: self._update_measurement_status(f"{label} running"))
                action()
                self.parent.after(0, lambda: self._update_measurement_status(f"{label} complete"))
            except Exception as exc:
                self._log(f"[Set] {label} error: {exc}")
                self.parent.after(
                    0,
                    lambda m=str(exc), title=label: messagebox.showerror(f"{title} Error", m),
                )
            finally:
                self._measurement_busy = False
    ...
    def _on_campaign_measure(self, condition: str) -> None:
        self._start_measurement_worker(
            self._measurement_condition_label(condition),
            lambda: self._campaign_capture_sync(condition),
        )

    def _on_measure_direct_point(self) -> None:
        self._start_measurement_worker(
            "AWG-DSO Direct",
            lambda: self._campaign_capture_sync("awg_dso_direct", sweep_kind="direct"),
        )

    def _on_measure_pilot_point(self) -> None:
        condition = (
            "si_only"
            if self.measurement_sweep_state_var.get().strip().lower().startswith("si")
            else "target_on"
        )
        self._start_measurement_worker(
            "Pilot Point",
            lambda: self._campaign_capture_sync(condition, sweep_kind="pilot"),
        )

    def _apply_campaign_dso_scales_sync(self) -> None:
        if not bool(self.live_var.get()):
            return
        with create_dso_controller(
            dso_type=normalize_dso_type(self.dso_type_var.get()),
            ...
            self._apply_dso_gui_settings_to_controller(
                dso,
                include_layout=False,
                touch_display=False,
                touch_scale=True,
    ...
    def _set_campaign_awg_vpp_sync(self, vpp: float) -> None:
        source = getattr(self, "tx_source", None)
        if source is None:
            if bool(self.live_var.get()):
                raise ValueError("The AWG panel is not connected to the DSO campaign.")
            return
        source.vpp_ch1_var.set(f"{vpp:.12g}")
        source.vpp_ch2_var.set(f"{vpp:.12g}")
        payload = self.runtime.get("tx_payload")
        if isinstance(payload, dict):
            payload["awg_ch1_vpp"] = float(vpp)
            payload["awg_ch2_vpp"] = float(vpp)
        if bool(self.live_var.get()):
            channels = parse_channels(source.ch_var.get()) or [1, 2]
            address = (
                f"TCPIP0::{source.ip_var.get().strip()}::"
    ...
    def _on_measure_direct_sweep(self) -> None:
        def action():
            vpp_values = self._parse_number_list(
                self.measurement_direct_vpp_var.get(), positive=True
            )
            scale_pairs = self._parse_scale_pairs(self.measurement_dso_scales_var.get())
            old_vpp_1 = old_vpp_2 = None
            source = getattr(self, "tx_source", None)
            if source is not None:
                old_vpp_1 = source.vpp_ch1_var.get()
                old_vpp_2 = source.vpp_ch2_var.get()
            old_scale_1 = self.ch1_scale_mv_var.get()
            old_scale_2 = self.ch2_scale_mv_var.get()
            try:
                total = len(vpp_values) * len(scale_pairs)
                index = 0
                for vpp in vpp_values:
                    self._set_campaign_awg_vpp_sync(vpp)
                    for scale_1, scale_2 in scale_pairs:
                        index += 1
                        self.ch1_scale_mv_var.set(f"{scale_1:g}")
                        self.ch2_scale_mv_var.set(f"{scale_2:g}")
                        self._apply_campaign_dso_scales_sync()
                        time.sleep(0.25)
                        self.parent.after(
                            0,
                            lambda i=index, n=total: self._update_measurement_status(
                                f"Direct sweep {i}/{n}"
                            ),
                        )
                        self._campaign_capture_sync(
                            "awg_dso_direct", sweep_kind="direct"
                        )
            finally:
                self.ch1_scale_mv_var.set(old_scale_1)
                self.ch2_scale_mv_var.set(old_scale_2)
                if source is not None and old_vpp_1 is not None:
                    source.vpp_ch1_var.set(old_vpp_1)
                    source.vpp_ch2_var.set(old_vpp_2)
                    if bool(self.live_var.get()):
                        try:
                            self._set_campaign_awg_vpp_sync(float(old_vpp_1))
                            self._apply_campaign_dso_scales_sync()
                        except Exception as exc:
                            self._log(f"[Set] Hardware restore warning: {exc}")
    ...
    def _download_campaign_payload_sync(self, npilot: int, rho: float) -> None:
        ...
        if callable(source.on_tx_generated):
            self.parent.after(
                0,
                lambda: source.on_tx_generated(str(APP_DIR / "data" / "current_tx_ref.npz")),
            )
        if bool(self.live_var.get()):
            channels = parse_channels(source.ch_var.get()) or [1, 2]
            address = (
            ...
            download_to_awg(
                awg_sig=np.asarray(payload["awg_sig"]),
                channels=channels,
                awg_addr=address,
                fs=float(payload["fs"]),
                vpp=source._vpp_by_channel(channels),
    ...
    def _on_measure_pilot_sweep(self) -> None:
        def action():
            source = getattr(self, "tx_source", None)
            if source is None:
                raise ValueError("The AWG TX panel is not connected to the DSO campaign.")
            np_values = [
                int(round(value))
                for value in self._parse_number_list(
                    self.measurement_np_list_var.get(), positive=True
                )
            ]
            if any(value < 8 for value in np_values):
                raise ValueError("Pilot Np must be at least 8.")
            rho_values = self._parse_number_list(self.measurement_rho_list_var.get())
            if any(value <= 0.0 or value >= 0.95 for value in rho_values):
                raise ValueError("rho sweep values must satisfy 0 < rho < 0.95.")
            condition = (
                "si_only"
                if self.measurement_sweep_state_var.get().strip().lower().startswith("si")
                else "target_on"
            ...
            try:
                for npilot in np_values:
                    for rho in rho_values:
                        ...
                        self._download_campaign_payload_sync(npilot, rho)
                        time.sleep(0.35)
                ...
                try:
                    ...
                    self._log(f"[Set] TX restore warning: {exc}")

        self._start_measurement_worker("Np/rho Sweep", action)

    def _campaign_post_sinr_db(self, record: dict[str, object]) -> float:
        for key in ("snr_rad_post_db_c2", "snr_rad_db"):
            value = self._measurement_metric_value(record, key)
            if np.isfinite(value):
                return float(value)
        for item in reversed(list(record.get("range_results", []))):
            try:
                value = float(item.get("range_profile_snr_db", float("nan")))
                if np.isfinite(value):
                    return value
            except Exception:
                pass
    ...
    def _show_campaign_calculation(self, calculation: str) -> None:
        try:
            if calculation == "snr":
                self._on_calculate_campaign_snr(show_dialog=True)
            elif calculation == "gp":
                self._on_estimate_processing_gain(show_dialog=True)
            elif calculation == "rcs":
                self._on_estimate_rcs(show_dialog=True)
            else:
                raise ValueError(f"Unknown campaign calculation: {calculation}")
        except Exception as exc:
            messagebox.showerror(
                "Measurement Calculation", str(exc), parent=self.parent
            )

    def _refresh_system_model_measurements(self) -> None:
        model = getattr(self, "system_model_source", None)
        if model is not None and hasattr(model, "_refresh_plot"):
            self.parent.after(0, model._refresh_plot)

    def _on_calculate_campaign_snr(self, *, show_dialog: bool = True) -> dict[str, float]:
        target = self._matching_measurement("target_on")
        if target is None:
            raise ValueError("A Target-on measurement is required.")
        receiver = self._matching_measurement(
            "receiver_noise", target, strict_waveform=True
        )
        si_only = self._matching_measurement("si_only", target, strict_waveform=True)
        if receiver is None:
            raise ValueError("A Receiver-noise measurement is required.")
        if si_only is None:
            raise ValueError(
                "A matching SI-only measurement is required at the same Np, rho, range, and photocurrent."
            )

        p_comm_on = self._measurement_raw_power_mw(target, sensing=False)
        p_comm_noise = self._measurement_raw_power_mw(receiver, sensing=False)
        p_sens_on = self._measurement_raw_power_mw(target, sensing=True)
        p_sens_si = self._measurement_raw_power_mw(si_only, sensing=True)
        if not all(np.isfinite(v) and v > 0.0 for v in (p_comm_on, p_comm_noise, p_sens_on, p_sens_si)):
            raise ValueError("The matched records do not contain valid C1/C2 raw band powers.")

        comm_signal = max(p_comm_on - p_comm_noise, 1e-300)
        target_excess = max(p_sens_on - p_sens_si, 1e-300)
        comm_snr_db = 10.0 * np.log10(comm_signal / p_comm_noise)
        sensing_pre_sinr_db = 10.0 * np.log10(target_excess / p_sens_si)
        result = {
            "comm_snr_from_rx_noise_db": float(comm_snr_db),
            "sensing_pre_sinr_db": float(sensing_pre_sinr_db),
            "target_excess_power_dbm": float(10.0 * np.log10(target_excess)),
            "si_only_baseline_power_dbm": float(10.0 * np.log10(p_sens_si)),
        }
        carrier = self._matching_measurement(
            "carrier_noise", target, strict_waveform=True
        )
        if carrier is not None:
            p_carrier = self._measurement_raw_power_mw(carrier, sensing=True)
            p_rx_sens = self._measurement_raw_power_mw(receiver, sensing=True)
            if np.isfinite(p_carrier) and np.isfinite(p_rx_sens):
                result["carrier_excess_power_dbm"] = float(
                    10.0 * np.log10(max(p_carrier - p_rx_sens, 1e-300))
                )
                result["si_excess_power_dbm"] = float(
                    10.0 * np.log10(max(p_sens_si - p_carrier, 1e-300))
                )
        self._measurement_results.update(result)
        target.setdefault("derived", {}).update(result)
        self._set_metric(
            "campaign_comm_snr_db",
            "Campaign Comm SNR",
            comm_snr_db,
            "dB",
            "Target-on C1 band excess divided by the UTC-PD-dark receiver-noise band power.",
        )
        self._set_metric(
            "campaign_sensing_pre_sinr_db",
            "Campaign Sensing Pre-DSP SINR",
            sensing_pre_sinr_db,
            "dB",
            "C2 Target-on minus matched SI-only, divided by the SI-only baseline.",
        )
        self._autosave_measurement_set()
        self._refresh_metrics_table()
        self._refresh_system_model_measurements()
        self._update_measurement_status(
            f"Comm SNR={comm_snr_db:.2f} dB, pre-DSP sensing={sensing_pre_sinr_db:.2f} dB"
        )
        if show_dialog:
            messagebox.showinfo(
                "Campaign SNR",
                f"Communication SNR: {comm_snr_db:.2f} dB\n"
                f"Sensing pre-DSP SINR: {sensing_pre_sinr_db:.2f} dB",
                parent=self.parent,
            )
        return result

    def _on_estimate_processing_gain(self, *, show_dialog: bool = True) -> float:
        estimates: list[dict[str, float]] = []
        for target in self._measurement_records:
            if str(target.get("condition", "")) != "target_on":
                continue
            si_only = self._matching_measurement(
                "si_only", target, strict_waveform=True
            )
            if si_only is None:
                continue
            p_on = self._measurement_raw_power_mw(target, sensing=True)
            p_si = self._measurement_raw_power_mw(si_only, sensing=True)
            post_db = self._campaign_post_sinr_db(target)
            try:
                rho = float(target.get("rho", float("nan")))
                npilot = float(target.get("pilot_np", float("nan")))
            except Exception:
                continue
            if not (
                np.isfinite(p_on)
                and np.isfinite(p_si)
                and p_on > p_si > 0.0
                and np.isfinite(post_db)
                and 0.0 < rho < 1.0
            ):
                continue
            pre_db = 10.0 * np.log10((p_on - p_si) / p_si)
            gp_db = post_db - pre_db - 10.0 * np.log10(rho)
            estimates.append(
                {
                    "pilot_np": npilot,
                    "rho": rho,
                    "pre_sinr_db": float(pre_db),
                    "post_sinr_db": float(post_db),
                    "gp_eff_db": float(gp_db),
                }
            )
        if not estimates:
            raise ValueError(
                "No matched SI-only/Target-on pairs with valid post-processing sensing SINR were found."
            )
        gp_values = np.asarray([item["gp_eff_db"] for item in estimates], dtype=np.float64)
        gp_db = float(np.median(gp_values))
        unique_np = sorted({item["pilot_np"] for item in estimates if item["pilot_np"] > 0})
        slope = float("nan")
        if len(unique_np) >= 2:
            grouped_x: list[float] = []
            grouped_y: list[float] = []
            for npilot in unique_np:
                values = [
                    item["post_sinr_db"]
                    for item in estimates
                    if item["pilot_np"] == npilot
                ]
                grouped_x.append(10.0 * np.log10(npilot))
                grouped_y.append(float(np.median(values)))
        ...
        if source is not None and hasattr(source, "params"):
            var = source.params.get("radar_proc_gain_eff_db")
            if var is not None:
                var.set(f"{gp_db:.6g}")
        model = getattr(self, "system_model_source", None)
        if model is not None and hasattr(model, "params"):
            var = model.params.get("radar_proc_gain_db")
            if var is not None:
                var.set(f"{gp_db:.6g}")
        self._set_metric(
        ...
        if np.isfinite(slope):
            self._set_metric(
                "processing_gain_np_slope",
                "Gp Np Slope",
                slope,
                "dB/dB",
                "Slope of post-processing sensing SINR versus 10log10(Np); ideal is one.",
            )
        self._autosave_measurement_set()
        self._refresh_metrics_table()
        self._refresh_system_model_measurements()
    ...
    def _on_estimate_rcs(self, *, show_dialog: bool = True) -> float:
        target = self._matching_measurement("target_on")
        if target is None:
            raise ValueError("A Target-on measurement is required.")
        si_only = self._matching_measurement(
        ...
        if si_only is None:
            raise ValueError("A matching SI-only measurement is required.")
        gp_db = float(
            self._measurement_results.get(
                "processing_gain_eff_db",
                self.runtime.get("measured_processing_gain_db", float("nan")),
            )
        )
        if not np.isfinite(gp_db):
            gp_db = self._on_estimate_processing_gain(show_dialog=False)
        post_db = self._campaign_post_sinr_db(target)
        rho = float(target.get("rho", float("nan")))
        baseline_mw = self._measurement_raw_power_mw(si_only, sensing=True)
        if not (
            np.isfinite(post_db)
            and 0.0 < rho < 1.0
            and np.isfinite(baseline_mw)
            and baseline_mw > 0.0
        ):
            raise ValueError("Valid post-SINR, rho, and SI-only baseline are required.")

        # Input-referred linear-equivalent target power.  The RX calibration
        # must include the complete THz receiver, square-law, IF, and DSO power
        # conversion when this is used with the present ZBD receiver.
        target_equiv_dso_mw = (
            baseline_mw
            * 10.0 ** (post_db / 10.0)
            / (rho * 10.0 ** (gp_db / 10.0))
        )
        rx_gain_db = float(self.measurement_rx_gain_db_var.get())
        received_mw = target_equiv_dso_mw / 10.0 ** (rx_gain_db / 10.0)
        tx_mw = 10.0 ** (float(self.measurement_tx_power_dbm_var.get()) / 10.0)
        gains = self._parse_number_list(self.measurement_ant_gain_var.get())
        if len(gains) != 2:
            raise ValueError("Enter TX and RX antenna gains as Gt, Gr.")
        gt = 10.0 ** (gains[0] / 10.0)
        gr = 10.0 ** (gains[1] / 10.0)
        loss = 10.0 ** (float(self.measurement_system_loss_db_var.get()) / 10.0)
        range_m = float(target.get("range_mm", 0.0)) * 1e-3
        rf_ghz = float(target.get("rf_ghz", float("nan")))
        if not np.isfinite(rf_ghz) or rf_ghz <= 0.0:
            raise ValueError("The matched Target-on record has no valid RF frequency.")
        if range_m <= 0.0 or tx_mw <= 0.0:
            raise ValueError("Range and TX output power must be positive.")
        wavelength_m = 299792458.0 / (rf_ghz * 1e9)
        sigma_m2 = (
            received_mw
            * (4.0 * np.pi) ** 3
            * range_m ** 4
            * loss
            / (tx_mw * gt * gr * wavelength_m ** 2)
        )
        rcs_dbsm = float(10.0 * np.log10(max(sigma_m2, 1e-300)))
        result = {
            "effective_rcs_m2": float(sigma_m2),
            "effective_rcs_dbsm": rcs_dbsm,
            "rcs_received_equiv_dbm": float(10.0 * np.log10(received_mw)),
            "rcs_rx_chain_gain_db": rx_gain_db,
            "rcs_method": "input-referred linear-equivalent monostatic radar equation",
        }
        self._measurement_results.update(result)
        target.setdefault("derived", {}).update(result)
        self.runtime["estimated_effective_rcs_dbsm"] = rcs_dbsm
        source = getattr(self, "simulation_source", None)
        if source is not None and hasattr(source, "params"):
            if "effective_rcs_dbsm" in source.params:
                source.params["effective_rcs_dbsm"].set(f"{rcs_dbsm:.6g}")
            if "target_rcs_mode" in source.params:
                source.params["target_rcs_mode"].set("Direct effective RCS")
        model = getattr(self, "system_model_source", None)
        if model is not None and hasattr(model, "params"):
            model.params["effective_rcs_dbsm"].set(f"{rcs_dbsm:.6g}")
        self._set_metric(
            "estimated_effective_rcs_dbsm",
            "Estimated Effective RCS",
            rcs_dbsm,
            "dBsm",
            "Input-referred linear-equivalent estimate; RX gain must calibrate the full square-law/IF/DSO conversion.",
        )
        self._autosave_measurement_set()
        self._refresh_metrics_table()
        self._refresh_system_model_measurements()
        self._update_measurement_status(f"RCS={rcs_dbsm:.2f} dBsm")
        if show_dialog:
            messagebox.showinfo(
                "Effective RCS Estimate",
                f"Effective RCS: {rcs_dbsm:.2f} dBsm\n"
                f"RCS: {sigma_m2:.4g} m^2\n\n"
                "RX gain must be an input-referred calibration of the complete "
                "receiver, square-law detector, IF chain, and DSO.",
                parent=self.parent,
            )
        return rcs_dbsm

    def _measurement_set_to_npz_items(self, *, include_raw: bool = True) -> dict[str, np.ndarray]:
        records = self._json_safe(self._measurement_records)
        out: dict[str, np.ndarray] = {
            "measurement_set_schema": np.asarray(["isac-measurement-set-v1"]),
            "measurement_set_name": np.asarray([self.measurement_set_name_var.get().strip()]),
            "measurement_set_created": np.asarray([datetime.now().isoformat(timespec="seconds")]),
            "measurement_record_count": np.asarray([len(self._measurement_records)], dtype=np.int64),
            "measurement_records_json": np.asarray([
                json.dumps(records, ensure_ascii=True, separators=(",", ":"))
            ]),
            "measurement_results_json": np.asarray([
                json.dumps(
                    self._json_safe(self._measurement_results),
                    ensure_ascii=True,
                    separators=(",", ":"),
                )
            ]),
        ...
        if include_raw:
            for rec_idx, record in enumerate(self._measurement_records):
                record_id = str(record.get("record_id", ""))
                for channel, item in self._measurement_raw.get(record_id, {}).items():
                    sig_key = f"measurement_raw__{rec_idx:04d}__{channel}__sig"
                    fs_key = f"measurement_raw__{rec_idx:04d}__{channel}__fs"
                    out[sig_key] = np.asarray(item["sig"], dtype=np.float32)
                    out[fs_key] = np.asarray([float(item["fs"])], dtype=np.float64)
                    raw_index.append(
                        {
                            "record_id": record_id,
                            "channel": channel,
        ...
        out["measurement_raw_index_json"] = np.asarray([
            json.dumps(raw_index, ensure_ascii=True, separators=(",", ":"))
        ])
        return out

    def _measurement_set_from_npz(self, loaded) -> bool:
        if "measurement_records_json" not in loaded.files:
            return False
        raw_records = str(self._npz_unpack_value(loaded["measurement_records_json"]))
        records = json.loads(raw_records)
        if not isinstance(records, list):
            raise ValueError("Invalid measurement set record list.")
        self._measurement_records = records
        self._measurement_results = {}
        if "measurement_results_json" in loaded.files:
            self._measurement_results = json.loads(
                str(self._npz_unpack_value(loaded["measurement_results_json"]))
            )
        self._measurement_raw = {}
        if "measurement_raw_index_json" in loaded.files:
            raw_index = json.loads(
                str(self._npz_unpack_value(loaded["measurement_raw_index_json"]))
            )
            for item in raw_index:
                if item["sig_key"] not in loaded.files or item["fs_key"] not in loaded.files:
                    continue
                self._measurement_raw.setdefault(item["record_id"], {})[item["channel"]] = {
                    "sig": np.asarray(loaded[item["sig_key"]], dtype=np.float32),
                    "fs": float(np.asarray(loaded[item["fs_key"]]).reshape(-1)[0]),
                }
        if "measurement_set_name" in loaded.files:
            self.measurement_set_name_var.set(
                str(self._npz_unpack_value(loaded["measurement_set_name"]))
            )
        self.runtime["measurement_set_records"] = self._measurement_records
        gp_db = self._measurement_results.get("processing_gain_eff_db")
        if gp_db is not None:
            try:
                self.runtime["measured_processing_gain_db"] = float(gp_db)
                self.radar_proc_gain_eff_var.set(f"{float(gp_db):.6g}")
                source = getattr(self, "simulation_source", None)
                if source is not None and hasattr(source, "params"):
                    var = source.params.get("radar_proc_gain_eff_db")
                    if var is not None:
                        var.set(f"{float(gp_db):.6g}")
                model = getattr(self, "system_model_source", None)
                if model is not None and hasattr(model, "params"):
                    model.params["radar_proc_gain_db"].set(f"{float(gp_db):.6g}")
            except Exception:
                pass
        rcs_dbsm = self._measurement_results.get("effective_rcs_dbsm")
        if rcs_dbsm is not None:
            try:
                self.runtime["estimated_effective_rcs_dbsm"] = float(rcs_dbsm)
                source = getattr(self, "simulation_source", None)
                if source is not None and hasattr(source, "params"):
                    if "effective_rcs_dbsm" in source.params:
                        source.params["effective_rcs_dbsm"].set(
                            f"{float(rcs_dbsm):.6g}"
                        )
                model = getattr(self, "system_model_source", None)
                if model is not None and hasattr(model, "params"):
                    model.params["effective_rcs_dbsm"].set(
                        f"{float(rcs_dbsm):.6g}"
                    )
            except Exception:
                pass
        self._update_measurement_status("loaded")
        self._refresh_system_model_measurements()
        return True

    def _write_measurement_set(self, path: Path, *, include_raw: bool = True) -> None:
        path.parent.mkdir(parents=True, exist_ok=True)
        np.savez_compressed(
            path, **self._measurement_set_to_npz_items(include_raw=include_raw)
        )

    def _autosave_measurement_set(self) -> None:
        if not self._measurement_records:
            return
        directory = APP_DIR / "data" / "measurement_sets"
        name = self._filename_safe_token(self.measurement_set_name_var.get())
        path = directory / f".autosave_{name}.npz"
        try:
            self._write_measurement_set(
                path, include_raw=bool(self.measurement_keep_raw_var.get())
            )
            self.runtime["latest_measurement_set_autosave"] = str(path)
        except Exception as exc:
            self._log(f"[Set] Autosave warning: {exc}")

    def _on_save_measurement_set(self) -> None:
        if not self._measurement_records:
            messagebox.showwarning("No Measurements", "The measurement set is empty.")
            return
        directory = APP_DIR / "data" / "measurement_sets"
        ...
        path_str = filedialog.asksaveasfilename(
            parent=self.parent,
            title="Save Measurement Set",
            initialdir=str(directory),
            initialfile=f"{name}.npz",
            defaultextension=".npz",
            filetypes=[("ISAC measurement set", "*.npz"), ("All files", "*.*")],
        )
        if not path_str:
            return
        try:
            self._write_measurement_set(Path(path_str), include_raw=True)
            self.runtime["latest_measurement_set_file"] = str(path_str)
            self._update_measurement_status(f"saved {Path(path_str).name}")
            self._log(f"[Set] Saved measurement set: {path_str}")
        except Exception as exc:
            messagebox.showerror("Save Measurement Set", str(exc), parent=self.parent)

    def _on_load_measurement_set(self) -> None:
        directory = APP_DIR / "data" / "measurement_sets"
        directory.mkdir(parents=True, exist_ok=True)
        path_str = filedialog.askopenfilename(
            parent=self.parent,
            title="Load Measurement Set",
            initialdir=str(directory),
            filetypes=[("ISAC measurement set", "*.npz"), ("All files", "*.*")],
        )
        if not path_str:
            return
        try:
            with np.load(path_str, allow_pickle=False) as loaded:
                if not self._measurement_set_from_npz(loaded):
                    raise ValueError("This file does not contain a measurement set.")
            self.runtime["latest_measurement_set_file"] = str(path_str)
            self._log(f"[Set] Loaded measurement set: {path_str}")
        except Exception as exc:
            messagebox.showerror("Load Measurement Set", str(exc), parent=self.parent)

    def _on_clear_measurement_set(self) -> None:
        if self._measurement_records and not messagebox.askyesno(
            "Clear Measurement Set",
            "Clear all in-memory measurement records and retained raw captures?",
            parent=self.parent,
        ):
            return
        ...
        self._measurement_raw = {}
        self._measurement_results = {}
        self.runtime.pop("measurement_set_records", None)
        self.runtime.pop("measured_processing_gain_db", None)
        self.runtime.pop("estimated_effective_rcs_dbsm", None)
        self._update_measurement_status("cleared")
        self._refresh_system_model_measurements()
    ...
    def _repeat_ensemble_to_npz_items(self) -> dict[str, np.ndarray]:
        ensemble = self._last_repeat_ensemble
        if not ensemble:
            return {}
        records = list(ensemble.get("records", []))
        stats = dict(ensemble.get("stats", {}))
        stat_keys = sorted(stats)
        stat_fields = ("count", "mean", "std", "min", "p25", "median", "p75", "max", "mad")
        matrix = np.asarray([
            [float(stats[key].get(field, float("nan"))) for field in stat_fields]
            for key in stat_keys
        ], dtype=np.float64)
        return {
            "repeat_schema": np.asarray([str(ensemble.get("schema", "isac-repeat-ensemble-v2"))]),
            "repeat_mode": np.asarray([str(ensemble.get("mode", ""))]),
            "repeat_count": np.asarray([len(records)], dtype=np.int64),
            "repeat_selected_index": np.asarray(
                [int(ensemble.get("selected_index", 0))], dtype=np.int64
            ),
            "repeat_records_json": np.asarray([
                json.dumps(records, ensure_ascii=True, separators=(",", ":"))
            ]),
            "repeat_stat_keys": np.asarray(stat_keys),
            "repeat_stat_fields": np.asarray(stat_fields),
            "repeat_stat_values": matrix,
        }

    def _repeat_ensemble_from_npz(self, loaded) -> None:
        if "repeat_records_json" not in loaded.files:
            self._last_repeat_ensemble = {}
            return
        try:
            raw = str(self._npz_unpack_value(loaded["repeat_records_json"]))
            records = json.loads(raw)
            stat_keys = [
                str(x.item() if hasattr(x, "item") else x)
                for x in np.asarray(loaded["repeat_stat_keys"] if "repeat_stat_keys" in loaded.files else []).reshape(-1)
            ]
            stat_fields = [
                str(x.item() if hasattr(x, "item") else x)
                for x in np.asarray(loaded["repeat_stat_fields"] if "repeat_stat_fields" in loaded.files else []).reshape(-1)
            ]
            values = np.asarray(
                loaded["repeat_stat_values"] if "repeat_stat_values" in loaded.files else [],
                dtype=np.float64,
            )
            stats = {
                key: {
                    field: float(values[i, j])
                    for j, field in enumerate(stat_fields)
                    if values.ndim == 2 and i < values.shape[0] and j < values.shape[1]
                }
                for i, key in enumerate(stat_keys)
            }
            self._last_repeat_ensemble = {
                "schema": str(self._npz_unpack_value(loaded["repeat_schema"])) if "repeat_schema" in loaded.files else "isac-repeat-ensemble-v2",
                "mode": str(self._npz_unpack_value(loaded["repeat_mode"])) if "repeat_mode" in loaded.files else "",
                "selected_index": int(self._npz_unpack_value(loaded["repeat_selected_index"])) if "repeat_selected_index" in loaded.files else 0,
    ...
    def _dsp_state_to_npz_items(self) -> dict:
        return {
            ...
            "dsp__radar_proc_gain_eff_db": np.asarray([self.radar_proc_gain_eff_var.get().strip()]),
    ...
    def _simulation_context_to_npz_items(self) -> dict[str, np.ndarray]:
        source = getattr(self, "simulation_source", None)
        if source is None or not hasattr(source, "_cfg_from_ui"):
            return {
                "metric_schema": np.asarray(["pilot-domain-sinr-v2"]),
            }
        try:
            cfg = source._cfg_from_ui()
            values = {
                key: value
            ...
            return {
                "metric_schema": np.asarray(["pilot-domain-sinr-v2"]),
                "simulation_config_json": np.asarray([
    ...
    def _range_results_to_npz_items(self, results: list[dict[str, object]] | None = None) -> dict:
        ...
        scalar_keys = {
            ...
            "range_profile_snr_db", "processing_gain_db", "ideal_processing_gain_db",
            "pilot_weighted_gain_db", "pg_corrected_snr_db",
        ...
        def worker():
            try:
                ...
                for ch, item in (self._rx_multi or {}).items():
                    key = ch.strip().upper()
                    try:
                        sig_i = np.asarray(item["sig"], dtype=np.float32)
                        fs_i = float(item["fs"])
                ...
                meta.update(self._simulation_context_to_npz_items())
                meta.update(self._metrics_to_npz_items())
                meta.update(self._repeat_ensemble_to_npz_items())
                meta.update(self._measurement_set_to_npz_items(include_raw=False))
    ...
    def _on_save_capture(self) -> None:
        ...
        def worker():
            try:
                path = Path(path_str)
                path.parent.mkdir(parents=True, exist_ok=True)
                sig = np.asarray(self._rx_sig, dtype=np.float32)
                fs = float(self._rx_fs)
                payload = self._load_tx_payload_for_isac()
                saved_channels = list(self._rx_multi.keys()) if self._rx_multi else [self.ch_var.get().strip()]
                primary_saved = self.ch_var.get().strip().upper()
                meta = {
                    "rx_sig": sig,
                    ...
                    "capture_schema": np.asarray(["isac-compact-capture-v2"]),
        ...
        def worker():
            try:
                ...
                with np.load(path, allow_pickle=True) as loaded:
                    if "rx_sig" not in loaded.files or "rx_fs" not in loaded.files:
                        ...
                        if range_items:
                            ...
                            self._repeat_ensemble_from_npz(loaded)
                            self._measurement_set_from_npz(loaded)
                    ...
                    primary_file_ch = str(primary_file_ch).strip().upper() or "C1"
                    rx_multi: dict[str, dict[str, np.ndarray | float]] = {
                        primary_file_ch: {"sig": sig, "t": t, "fs": fs}
                    }
                    ...
                    gui_vals = {
                        ...
                        "radar_proc_gain_eff_db": self._npz_unpack_value(loaded["dsp__radar_proc_gain_eff_db"]) if "dsp__radar_proc_gain_eff_db" in loaded.files else None,
                ...
                def update_ui():
                    ...
                    if gui_vals["radar_proc_gain_eff_db"] is not None:
                        self.radar_proc_gain_eff_var.set(str(gui_vals["radar_proc_gain_eff_db"]))
    ...
    def _compute_isac_range_profile_for_signal(self, sig: np.ndarray, fs: float,
        ...
        corr_power_acc = np.zeros(
            pts_per_chirp + ref_len - 1, dtype=np.float64
        )
        for i in range(n_chirps):
            ref_i = ref_mat[min(i, ref_mat.shape[0] - 1)]
            ci = fftconvolve(
                rx_mat[i], np.conj(ref_i[::-1]), mode="full"
            )
            corr_power_acc += np.abs(ci) ** 2
        corr_power_acc /= max(n_chirps, 1)
        corr_acc = np.sqrt(np.maximum(corr_power_acc, 0.0))
        ...
        range_profile_contrast_db = float("nan")
        range_floor_iqr_db = float("nan")
        processing_gain_db = float("nan")
        ideal_processing_gain_db = float("nan")
        pilot_weighted_gain_db = float("nan")
        pg_corrected_snr_db = float("nan")
        try:
            f1_ghz, f2_ghz = self._get_signal_band_ghz()
            occupied_bw_hz = max((f2_ghz - f1_ghz) * 1e9, 1.0)
            pilot_time_s = max(float(ref_len) / max(float(fs_ref), 1.0), 1e-15)
            ideal_processing_gain_db = 10.0 * math.log10(
                max(occupied_bw_hz * pilot_time_s, 1.0)
            )
        ...
        try:
            if len(prof_db) == len(rng) and len(prof_db) > 8 and len(prof_db) > est_idx:
                ...
                if np.count_nonzero(side) >= 8:
                    side_power = np.asarray(
                        corr_power_acc[side], dtype=np.float64
                    )
                    noise_power = float(np.nanmedian(side_power))
                    peak_power = float(corr_power_acc[est_idx])
                    if noise_power > 0.0 and peak_power > 0.0:
                        peak_to_floor = peak_power / noise_power
                        range_profile_contrast_db = 10.0 * math.log10(
                            peak_to_floor
                        )
                        range_profile_snr_db = 10.0 * math.log10(
                            max(peak_to_floor - 1.0, 1e-15)
                        )
                    side_power_db = 10.0 * np.log10(
                        np.maximum(side_power, 1e-300)
                    )
                    q25, q75 = np.nanpercentile(
                        side_power_db, [25.0, 75.0]
                    )
                    range_floor_iqr_db = float(q75 - q25)
                    if np.isfinite(pilot_weighted_gain_db):
                        pg_corrected_snr_db = (
                            range_profile_snr_db - pilot_weighted_gain_db
                        )
        except Exception:
            range_profile_snr_db = float("nan")
            range_profile_contrast_db = float("nan")
    ...
    def _show_isac_range_results(self, results: list[dict]) -> None:
        ...
        for item in results:
            ...
            range_profile_contrast_db = float(
                item.get("range_profile_contrast_db", range_profile_snr_db)
            )
            range_floor_iqr_db = float(
                item.get("range_floor_iqr_db", float("nan"))
            )
            processing_gain_db = float(item.get("processing_gain_db", float("nan")))
            ideal_processing_gain_db = float(item.get("ideal_processing_gain_db", float("nan")))
            pilot_weighted_gain_db = float(item.get("pilot_weighted_gain_db", float("nan")))
...
class SystemModelValidationPanel:
    ...
    def _float_pair(
        ...
        """Parse compact ``min, max`` controls used by the validation tab."""
        try:
            raw = self.params[key].get().strip().replace(";", ",")
    ...
    def _required_sinr_db(self) -> tuple[float, float]:
        """Return communication and sensing thresholds, respectively."""
        return self._float_pair("required_sinr_db", (15.75, 13.2))

    def _ideal_processing_gain_lin(self) -> float:
    ...
    def _sensing_residual_parameters(self) -> tuple[float, float]:
        """Return residual ceiling [dB] and SI-quadratic noise fraction."""
        ceiling_db, quadratic_fraction = self._float_pair(
            "sensing_residual_model", (40.0, 0.10)
        )
        ceiling_db = float(np.clip(ceiling_db, -100.0, 300.0))
        quadratic_fraction = float(np.clip(quadratic_fraction, 0.0, 1.0))
        return ceiling_db, quadratic_fraction

    def _sensing_residual_ceiling_lin(self) -> float:
        ceiling_db, _fraction = self._sensing_residual_parameters()
        return max(10.0 ** (ceiling_db / 10.0), 1e-30)

    def _apply_sensing_residual_ceiling(
        self, raw_sinr: np.ndarray | float
    ) -> np.ndarray:
        """Combine detector-limited SINR with the sensing residual-SIR ceiling."""
        raw = np.maximum(np.asarray(raw_sinr, dtype=np.float64), 0.0)
        ceiling = self._sensing_residual_ceiling_lin()
        return 1.0 / (
            1.0 / np.maximum(raw, 1e-300)
            + 1.0 / max(ceiling, 1e-300)
        )

    def _required_raw_sensing_sinr(self, effective_threshold: float) -> float:
        """Map an effective SINR threshold through the residual-SIR ceiling."""
        threshold = max(float(effective_threshold), 1e-30)
        ceiling = self._sensing_residual_ceiling_lin()
        if ceiling <= threshold * (1.0 + 1e-12):
            return float("inf")
        return float(1.0 / (1.0 / threshold - 1.0 / ceiling))

    def _detector_output_noise_mw(
        self, si_power_scale: np.ndarray | float = 1.0
    ) -> np.ndarray:
        """Interpolate calibrated C2 noise and residual interference versus SI.

        The SI-on detector-output reference already contains receiver noise,
        SI--noise beating, residual waveform interference, IF-chain noise, and
        DSO noise.  The positive SI-on/off difference is split into a linear
        SI--noise term and a quadratic residual term while preserving the
        measured reference exactly at scale=1:

          N(s)=N0+dN*((1-eta)*s + eta*m4*s^2).

        ``eta`` is the user-entered quadratic fraction at the reference point,
        and m4 follows the relative m^4=(1/CSPR)^2 scaling.  This gives the
        expected high-SI roll-off without inventing a second absolute noise
        reference.
        """
        ...
        _ceiling_db, quadratic_fraction = self._sensing_residual_parameters()
        cspr_db = self._float("cspr_db", 13.0)
        cspr_ref_db = self._float("detector_ref_cspr_db", cspr_db)
        m4_scale = 10.0 ** (2.0 * (cspr_ref_db - cspr_db) / 10.0)
        si_dependent = si_excess_ref_mw * (
            (1.0 - quadratic_fraction) * scale
            + quadratic_fraction * m4_scale * scale ** 2
        )
        return np.asarray(
            baseline_mw + si_dependent, dtype=np.float64
        )
    ...
    def _theory_link_terms(self) -> dict[str, float]:
        """Return the common, input-referred terms used by every range equation."""
        return {
            "cspr": 10.0 ** (self._float("cspr_db", 20.0) / 10.0),
            "alpha": 10.0 ** (-self._theory_isolation_db() / 20.0),
            "ac2_mw": self._theory_carrier_power_mw(),
    ...
    def _solve_sensing_range(
        ...
        """Solve gamma*R^8-C4*R^4-C8=0 for the positive range."""
        c4_arr = np.maximum(np.asarray(c4, dtype=np.float64), 0.0)
        c8_arr = np.maximum(np.asarray(c8, dtype=np.float64), 0.0)
        gamma = max(float(gamma_threshold), 1e-30)
        if not np.isfinite(gamma):
            shape = np.broadcast_arrays(c4_arr, c8_arr)[0].shape
            return np.zeros(shape, dtype=np.float64)
        r4 = (c4_arr + np.sqrt(c4_arr ** 2 + 4.0 * gamma * c8_arr)) / (2.0 * gamma)
        return np.maximum(r4, 0.0) ** 0.25
    ...
    def _model(self, ranges_m: np.ndarray, rho: float | None = None) -> dict[str, np.ndarray | float]:
        ...
        raw_snr_sens = c4 / r ** 4 + c8 / r ** 8
        snr_sens = self._apply_sensing_residual_ceiling(raw_snr_sens)
        return {
            "rho": rho_v,
            "snr_comm": snr_comm,
            "snr_sens": snr_sens,
    ...
    def _rmax(self, rho: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        comm_threshold_db, sens_threshold_db = self._required_sinr_db()
        g_sens = self._required_raw_sensing_sinr(
            10.0 ** (sens_threshold_db / 10.0)
        )
    ...
    def _rmax_sensing_without_si(self, rho: np.ndarray) -> np.ndarray:
        """Noise-limited echo-self-beat range from the Sec. II equation."""
        rho = np.clip(np.asarray(rho, dtype=np.float64), 1e-12, 1.0 - 1e-12)
        _comm_threshold_db, threshold_db = self._required_sinr_db()
        gamma_threshold = self._required_raw_sensing_sinr(
    ...
    def _detector_domain_sensing_coefficients(
        ...
        calibration_scale = 10.0 ** (
            self._float("sensing_calibration_db", 0.0) / 10.0
        )
        common_scale = rho_scale * tx_scale * modulation_scale * calibration_scale
    ...
    def _build_ui(self) -> None:
        ...
        # The parameter list is taller than a typical laptop display.  Keep
        # the plot pane fixed and make the controls independently scrollable
        # so the action buttons remain reachable at every screen resolution.
        control_canvas = tk.Canvas(left, width=300, highlightthickness=0, borderwidth=0)
        control_scrollbar = ttk.Scrollbar(left, orient=tk.VERTICAL, command=control_canvas.yview)
        control_canvas.configure(yscrollcommand=control_scrollbar.set)
        control_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        control_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        control_host = ttk.Frame(control_canvas)
        control_window = control_canvas.create_window((0, 0), window=control_host, anchor="nw")

        def resize_control_scrollregion(_event=None) -> None:
            control_canvas.configure(scrollregion=control_canvas.bbox("all"))

        def resize_control_width(event) -> None:
            control_canvas.itemconfigure(control_window, width=max(int(event.width), 1))

        def scroll_controls(event) -> None:
            widget = event.widget
            while widget is not None and widget is not control_host and widget is not control_canvas:
                widget = getattr(widget, "master", None)
            if widget is control_host or widget is control_canvas:
                delta = -1 if event.delta > 0 else 1
                control_canvas.yview_scroll(delta, "units")

        control_host.bind("<Configure>", resize_control_scrollregion)
        control_canvas.bind("<Configure>", resize_control_width)
        control_canvas.bind_all("<MouseWheel>", scroll_controls, add="+")

        ctrl = ttk.LabelFrame(control_host, text="ISAC Range Validation", padding=8)
        ...
        def add(row: int, key: str, label: str, value: str, read_only: bool = False) -> None:
            ...
            self.params[key] = var
            entry = ttk.Entry(ctrl, textvariable=var, width=14)
        ...
        add(1, "ref_range_m", "Reference / TX-curve range [m]", "1.0")
        add(2, "sweep_tx_power_dbm", "Operating THz P_t [dBm]", "-10")
        add(3, "si_sweep_limits_dbm", "SI sweep [dBm] min, max", "-60, -20")
        add(4, "si_on_iso_db", "Net SI isolation [dB]", "24")
        add(5, "lna_ip1db_dbm", "LNA input P1dB [dBm]", "-20")
        add(6, "rho", "Power allocation rho", "0.20")
        add(7, "effective_rcs_dbsm", "Effective RCS [dBsm]", "-6.0")
        add(8, "radar_proc_gain_db", "Sensing G_p,eff [dB]", "26.0")
        add(9, "required_sinr_db", "Req. SINR comm, sensing [dB]", "15.75, 13.2")
        add(10, "sinr_ylim_db", "SINR y limits [dB]", "0, 40")
        add(11, "rcs_limits_dbsm", "RCS sweep [dBsm] min, max", "-30, 0")
        add(12, "sensing_calibration_db", "Practical sensing calibration [dB]", "0.0")
        add(13, "sensing_residual_model", "Practical ceiling [dB], SI^2 fraction", "40, 0.10")
        add(14, "manual_range_evm_points", "Measured EVM [mm:dB]", "1000:-17.49, 1100:-16.21, 1200:-15.10")
        add(15, "manual_evm_points", "Measured EVM [GBd:dB]", "2:-25.56, 4:-23.37, 8:-20.46, 10:-19.5, 12:-18.61, 15:-17.49, 17:-16.5, 20:-15.8")
        add(16, "manual_c2_si_on_points", "Measured C2 [mm:dBm]", "1000:-38.3, 1100:-40.6, 1200:-42.4")
        hidden("r_min_m", "0.5")
        hidden("r_max_m", "2")
        hidden("sweep_points", "9")
        hidden("si_off_iso_db", "1000")
        hidden("system_nf_db", "8.0")
        hidden("bandwidth_ghz", "15.0")
        hidden("theoretical_snr_anchor_db", "23.55")
        hidden("pilot_symbols", "1024")
        hidden("comm_req_snr_db", "15.75")
        hidden("sens_req_snr_db", "13.2")
        hidden("sinr_ylim_min", "0")
        hidden("sinr_ylim_max", "40")
        hidden("radar_ylim_min", "0")
        hidden("radar_ylim_max", "40")
        hidden("ideal_processing_gain_db", "30.10")
        hidden("isac_range_ylim_min", "")
        hidden("isac_range_ylim_max", "")
        hidden("rcs_min_dbsm", "-30")
        hidden("rcs_max_dbsm", "0")
        ...
        hidden("si_sweep_points", "201")
        # Table-I normalization for the two closed-form theory figures.
        # Detector conversion kappa_d is fixed once at this point and is not
        # refitted while SI power, RCS, or range is swept.
        hidden("theory_anchor_sinr_db", "17.49, 27.0")
        hidden("theory_anchor_range_m", "1.0")
        hidden("theory_anchor_rcs_dbsm", "-4.0")
        hidden("theory_anchor_tx_dbm", "-10.0")
        hidden("theory_anchor_iso_db", "24.0")
        hidden("theory_anchor_rho", "0.20")
        hidden("theory_anchor_cspr_db", "13.0")
        hidden("theory_anchor_noise_dbm", "-64.2")
        ...
        hidden("c2_spectrum_noise_power_dbm", "")
        ...
        action_btns.grid(row=18, column=0, columnspan=2, sticky="ew", pady=(6, 0))
        ...
        photocurrent_btns.grid(row=19, column=0, columnspan=2, sticky="ew", pady=(6, 0))
        ttk.Button(
            photocurrent_btns,
        ...
        ).grid(row=20, column=0, columnspan=2, sticky="w", pady=(2, 0))
        ...
        self.canvas = FigureCanvasTkAgg(self.fig, master=right)
        self.canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        self._si_cursor_axes = None
        self._si_cursor_x = np.asarray([], dtype=np.float64)
        self._si_cursor_y = np.asarray([], dtype=np.float64)
        self._si_cursor_marker = None
        self._si_cursor_annotation = None
        self._rcs_cursor_axes = None
        self._rcs_cursor_series = []
        self._rcs_cursor_marker = None
        self._rcs_cursor_annotation = None
        self.canvas.mpl_connect(
            "motion_notify_event", self._on_si_cursor_motion
        )
        self.canvas.mpl_connect(
            "motion_notify_event", self._on_rcs_cursor_motion
        )
        self.canvas.mpl_connect(
            "figure_leave_event", self._hide_si_cursor
        )
        self.canvas.mpl_connect(
            "figure_leave_event", self._hide_rcs_cursor
        )
    ...
    def _load_packaged_detector_reference(
        ...
        try:
            ...
            for key, value in reference.items():
                if key == "effective_rcs_dbsm":
                    # This is the current scenario input synchronized from
                    # the first tab. Keep it independent from the packaged
                    # detector-reference RCS used for analytical scaling.
                    continue
    ...
    def _apply_evm_radar_limits(self, ax_sinr, ax_radar, x_min: float, x_max: float) -> None:
        ...
        for line in list(ax_sinr.get_lines()) + list(ax_radar.get_lines()):
            ...
            positive_x.extend(
                values[np.isfinite(values) & (values > 0.0)].tolist()
            )
        log_min = x_min if x_min > 0.0 else (
            min(positive_x) if positive_x else 1e-3
        )
        log_max = (
            x_max
            if x_max > log_min
            else max(positive_x, default=log_min * 10.0)
        )
        ...
        ticks = np.asarray(
            sorted({
                ...
                for value in (multiplier * (10.0 ** decade),)
                if log_min * (1.0 - 1e-10)
                <= value
                <= log_max * (1.0 + 1e-10)
            }),
            dtype=np.float64,
        ...
        if not len(ticks):
            ticks = np.asarray([log_min, log_max], dtype=np.float64)
        ax_sinr.xaxis.set_major_locator(FixedLocator(ticks))
        ax_sinr.xaxis.set_major_formatter(
            FuncFormatter(lambda value, _pos: f"{value:g}")
        )
        ax_sinr.xaxis.set_minor_formatter(NullFormatter())
        ax_radar.set_xlabel("Range (m, log scale)")

        y_min, y_max = self._float_pair("sinr_ylim_db", (0.0, 40.0))
        if y_max > y_min:
            ax_sinr.set_ylim(y_min, y_max)
            ax_radar.set_ylim(y_min, y_max)
            first_tick = 10.0 * np.ceil(y_min / 10.0)
            last_tick = 10.0 * np.floor(y_max / 10.0)
            ax_radar.yaxis.set_major_locator(
                FixedLocator(
                    np.arange(first_tick, last_tick + 5.0, 10.0)
                )
            )
    ...
    def _draw_symbol_rate_axis(self, ax, cache: dict[str, object], for_save: bool = False) -> None:
        ...
        if not for_save and len(sim_rate) and len(sim_rate) == len(bound_evm):
            ...
            # The legacy helper returns the non-AWGN EVM excess.  Measurements
            # do not identify that excess as SSBI alone, so treat it as total
            # residual interference (SSBI plus implementation residuals).
            residual_excess = tradeoff._ssbi_excess_evm_power(
                sim_rate, bound_evm, 4.0
            )
            sinr_evm = tradeoff._evm_power_to_db(
                tradeoff._evm_db_to_power(snr_at_sim_rate) + residual_excess
            )
            sinr_line, = ax.plot(
                sim_rate,
                sinr_evm,
        ...
        comm_threshold_db, _sens_threshold_db = self._required_sinr_db()
        ax.axhline(-comm_threshold_db, color="#444444", linestyle=":", linewidth=1.0)
        ax.set_xscale("log")
    ...
    def _closed_form_theory_context(self) -> dict[str, float]:
        """Return the fixed normalization and current Eq. (9) parameters.

        The manuscript leaves the detector power-product conversion
        ``kappa_d`` unspecified.  Fix it once at the Table-I reference point,
        then keep it constant while SI, RCS, and range are swept.  Powers are
        represented by their numerical values in mW, matching the manuscript
        normalization of N, P_SI, and P_ec.
        """
        tx_dbm = self._float("sweep_tx_power_dbm", -10.0)
        ...
        gp = max(self._ideal_processing_gain_lin(), 1.0)
        noise_mw = max(self._noise_power_mw(), 1e-30)

        # Echo power at R=1 m, sigma=1 m^2, and Pt=1 mW.  This is a
        # deterministic radar-equation coefficient, not a detector simulation
        # or measured range fit.
        echo_per_pt_unit_rcs = float("nan")
        try:
            if (
                self.photonic_source is not None
                ...
                link = calc_isac_link_budget(
                    ...
                    target_gamma_mag=cfg.target_gamma_mag,
                    target_pol_eff=cfg.target_pol_eff,
                    effective_rcs_dbsm=0.0,
                )
                echo_per_pt_unit_rcs = 10.0 ** (
                    float(link["c2_rf_dbm"]) / 10.0
                )
        except Exception:
            echo_per_pt_unit_rcs = float("nan")
        if not np.isfinite(echo_per_pt_unit_rcs):
            nominal_rcs = 10.0 ** (
                self._float("effective_rcs_dbsm", -6.0) / 10.0
            )
            echo_per_pt_unit_rcs = (
                max(self._float("sqrt_k", 1e-4), 1e-30) ** 2
                / max(nominal_rcs, 1e-30)
            )

        comm_anchor_db, sensing_anchor_db = self._float_pair(
            "theory_anchor_sinr_db", (17.49, 27.0)
        )
        ref_range = max(
            self._float("theory_anchor_range_m", 1.0), 1e-12
        )
        ref_rcs = 10.0 ** (
            self._float("theory_anchor_rcs_dbsm", -4.0) / 10.0
        )
        ref_pt_mw = 10.0 ** (
            self._float("theory_anchor_tx_dbm", -10.0) / 10.0
        )
        ref_si_mw = ref_pt_mw * 10.0 ** (
            -self._float("theory_anchor_iso_db", 24.0) / 10.0
        )
        ref_rho = float(
            np.clip(self._float("theory_anchor_rho", 0.20), 1e-12, 1.0)
        )
        ref_m2 = 10.0 ** (
            -self._float("theory_anchor_cspr_db", 13.0) / 10.0
        )
        ref_noise_mw = 10.0 ** (
            self._float("theory_anchor_noise_dbm", -64.2) / 10.0
        )
        ref_gp = 1024.0
        ref_echo_mw = (
            echo_per_pt_unit_rcs
        ...
        ref_denominator = (
            ref_noise_mw
            + 2.0 * ref_si_mw * ref_noise_mw
            + ref_m2 ** 2 * ref_si_mw ** 2
        )
        ref_shape = (
            ref_si_mw * ref_echo_mw + ref_echo_mw ** 2
        ) / max(ref_denominator, 1e-300)
        sensing_anchor = 10.0 ** (sensing_anchor_db / 10.0)
        kappa_d = sensing_anchor / max(
            ref_rho * ref_gp * 2.0 * ref_m2 * ref_shape,
            1e-300,
        )
        ...
        return {
            ...
            "noise_mw": noise_mw,
            "si_mw": si_mw,
            "echo_per_pt_unit_rcs": echo_per_pt_unit_rcs,
            "kappa_d": kappa_d,
            "comm_anchor_lin": 10.0 ** (comm_anchor_db / 10.0),
            "sensing_anchor_db": sensing_anchor_db,
    ...
    def _si_power_sweep_curves(self) -> dict[str, np.ndarray | float]:
        """Evaluate manuscript Eq. (9) at fixed range and echo power."""
        context = self._closed_form_theory_context()
        p_min, p_max = self._float_pair(
            "si_sweep_limits_dbm", (-60.0, -20.0)
        )
        if p_max <= p_min:
            p_max = p_min + 1.0
        points = max(32, min(self._int("si_sweep_points", 201), 4001))
        ...
        thermal = np.full_like(p_si_mw, noise_mw)
        si_noise = 2.0 * p_si_mw * noise_mw
        ssbi = m2 ** 2 * p_si_mw ** 2
        denominator = thermal + si_noise + ssbi
        with_si = prefactor * (
            p_si_mw * echo_mw + echo_mw ** 2
        ) / np.maximum(denominator, 1e-300)
        without_si = np.full_like(
            p_si_mw,
            prefactor * echo_mw ** 2 / max(noise_mw, 1e-300),
        )
        # Cross-beat-only thermal-noise asymptote from Eq. (10).  Relative to
        # the SI-off self-beat floor, its vertical separation is
        # 10log10(P_SI/P_ec), i.e. the ideal homodyne gain.
        ideal_homodyne = (
            prefactor * p_si_mw * echo_mw / max(noise_mw, 1e-300)
        )

        with_si_db = 10.0 * np.log10(np.maximum(with_si, 1e-300))
        local_slope = np.gradient(with_si_db, p_si_dbm)
        linear_mask = (
            np.isfinite(local_slope)
            & (local_slope >= 0.90)
            & (local_slope <= 1.05)
        )
        linear_indices = np.flatnonzero(linear_mask)
        linear_min_dbm = float("nan")
        linear_max_dbm = float("nan")
        if len(linear_indices):
            breaks = np.flatnonzero(np.diff(linear_indices) > 1) + 1
            runs = np.split(linear_indices, breaks)
            longest = max(runs, key=len)
            linear_min_dbm = float(p_si_dbm[int(longest[0])])
            linear_max_dbm = float(p_si_dbm[int(longest[-1])])
        current_si_dbm = 10.0 * np.log10(max(context["si_mw"], 1e-300))
        current_sinr_db = float(
            np.interp(current_si_dbm, p_si_dbm, with_si_db)
        )
        valid = p_si_dbm <= self._float("lna_ip1db_dbm", -20.0)
        valid_values = np.where(valid, with_si, np.nan)
        optimum_index = (
            int(np.nanargmax(valid_values))
            if np.any(np.isfinite(valid_values))
            else int(np.argmax(with_si))
        )
        return {
            "si_power_dbm": p_si_dbm,
            "with_si_db": with_si_db,
            "without_si_db": 10.0
            * np.log10(np.maximum(without_si, 1e-300)),
            "ideal_homodyne_db": 10.0
            * np.log10(np.maximum(ideal_homodyne, 1e-300)),
            "thermal_term": thermal,
            "si_noise_term": si_noise,
            "ssbi_term": ssbi,
            "current_si_dbm": current_si_dbm,
            "current_sinr_db": current_sinr_db,
            "compression_si_dbm": self._float("lna_ip1db_dbm", -20.0),
            "optimum_si_dbm": float(p_si_dbm[optimum_index]),
            "optimum_sinr_db": float(with_si_db[optimum_index]),
            "echo_power_dbm": float(
                10.0 * np.log10(max(echo_mw, 1e-300))
            ),
            "homodyne_gain_db": float(
                current_si_dbm
                - 10.0 * np.log10(max(echo_mw, 1e-300))
            ),
            "linear_region_min_dbm": linear_min_dbm,
            "linear_region_max_dbm": linear_max_dbm,
            "plot_range_m": plot_range,
            "rcs_dbsm": rcs_dbsm,
            "ideal_processing_gain_db": float(
                10.0 * np.log10(max(context["gp"], 1e-300))
            ),
            "rho": context["rho"],
        }

    def _hide_si_cursor(self, _event=None) -> None:
        marker = getattr(self, "_si_cursor_marker", None)
        annotation = getattr(self, "_si_cursor_annotation", None)
        changed = False
    ...
    def _on_si_cursor_motion(self, event) -> None:
        """Show a screen-only nearest-x marker on the SI theory curve."""
        ax = getattr(self, "_si_cursor_axes", None)
        marker = getattr(self, "_si_cursor_marker", None)
        annotation = getattr(self, "_si_cursor_annotation", None)
        x = np.asarray(
            getattr(self, "_si_cursor_x", []), dtype=np.float64
        )
        y = np.asarray(
            getattr(self, "_si_cursor_y", []), dtype=np.float64
        )
        if (
            ax is None
            or marker is None
        ...
        annotation.set_text(
            f"$P_{{\\mathrm{{SI}}}}={x_value:.1f}$ dBm\n"
            f"SINR$={y_value:.2f}$ dB"
        )
        x_mid = 0.5 * sum(ax.get_xlim())
        annotation.set_position(
            (-10, 10) if x_value > x_mid else (10, 10)
        )
        annotation.set_ha("right" if x_value > x_mid else "left")
        annotation.set_visible(True)
        self.canvas.draw_idle()

    def _hide_rcs_cursor(self, _event=None) -> None:
        marker = getattr(self, "_rcs_cursor_marker", None)
        annotation = getattr(self, "_rcs_cursor_annotation", None)
        changed = False
        if marker is not None and marker.get_visible():
            marker.set_visible(False)
            changed = True
    ...
    def _on_rcs_cursor_motion(self, event) -> None:
        """Show the nearest RCS-range curve value using display coordinates."""
        ax = getattr(self, "_rcs_cursor_axes", None)
        marker = getattr(self, "_rcs_cursor_marker", None)
        annotation = getattr(self, "_rcs_cursor_annotation", None)
        series = getattr(self, "_rcs_cursor_series", [])
        if (
            ax is None
            or marker is None
            or annotation is None
        ...
        for label, x_values, y_values, color in series:
            ...
            idx = int(
                valid_indices[
                    np.argmin(np.abs(x[valid_indices] - float(event.xdata)))
                ]
            )
            x_value = float(x[idx])
            y_value = float(y[idx])
            ...
            distance = float(
                np.hypot(display_x - float(event.x), display_y - float(event.y))
            )
            if distance < nearest_distance:
                nearest_distance = distance
                nearest = (label, x_value, y_value, color)

        if nearest is None:
            self._hide_rcs_cursor()
            return

        label, x_value, y_value, color = nearest
        marker.set_data([x_value], [y_value])
        ...
        annotation.set_text(
            f"{label}\n"
            f"RCS = {x_value:.1f} dBsm\n"
            f"Range = {y_value:.3g} m"
        )
        x_mid = 0.5 * sum(ax.get_xlim())
        y_mid = float(np.sqrt(np.prod(ax.get_ylim())))
        annotation.set_position(
            (
                -10 if x_value > x_mid else 10,
                -10 if y_value > y_mid else 10,
            )
        )
        annotation.set_ha("right" if x_value > x_mid else "left")
        annotation.set_va("top" if y_value > y_mid else "bottom")
        annotation.set_visible(True)
        self.canvas.draw_idle()

    def _legacy_tx_power_sweep_curves(self) -> dict[str, np.ndarray | float]:
        """Return detector-calibrated sensing SINR versus total THz TX power.
        ...
        TX sweep changes SI and echo together: P_SI and P_echo are both
        proportional to P_t, both target detector powers are proportional to
        P_t^2, and the calibrated SI-induced noise increment is proportional
        to P_t.  A reference-preserving quadratic residual term scales as
        m^4*P_SI^2, and the result is combined with the configured sensing
        residual-interference ceiling.
        """
        tx_min, tx_max = self._float_pair(
            "tx_sweep_limits_dbm", (-35.0, 5.0)
        )
        if tx_max <= tx_min:
            tx_max = tx_min + 1.0
        points = max(32, min(self._int("tx_sweep_points", 201), 4001))
        tx_power_dbm = np.linspace(tx_min, tx_max, points)
        p_si_dbm = self._tx_dbm_to_si_input_dbm(tx_power_dbm)
        p_si_mw = 10.0 ** (p_si_dbm / 10.0)
        ...
        si_scale = np.asarray(
            self._detector_si_power_scale(tx_power_dbm),
            dtype=np.float64,
        )
        si_ref_mw = float(
            10.0
            ** (
                self._tx_dbm_to_si_input_dbm(tx_ref_dbm)
                / 10.0
            )
        )
        ...
        if references_valid:
            ...
            detector_noise_off_mw = float(
                self._detector_output_noise_mw(0.0)
            )
            cross_sinr = (
            ...
            cross_sinr *= calibration_scale
            self_sinr *= calibration_scale
            raw_full_sinr = cross_sinr + self_sinr
            raw_without_si_sinr = (
                pilot_weighted_gain
                * self_target_mw
                / max(detector_noise_off_mw, 1e-300)
                * calibration_scale
            )
            full_sinr = self._apply_sensing_residual_ceiling(raw_full_sinr)
            without_si_sinr = self._apply_sensing_residual_ceiling(
                raw_without_si_sinr
            )
        else:
            cross_sinr = np.full_like(p_si_mw, np.nan)
            self_sinr = np.full_like(p_si_mw, np.nan)
            raw_full_sinr = np.full_like(p_si_mw, np.nan)
            raw_without_si_sinr = np.full_like(p_si_mw, np.nan)
            full_sinr = np.full_like(p_si_mw, np.nan)
            without_si_sinr = np.full_like(p_si_mw, np.nan)
        ...
        carrier_tx_mw = (
            ...
            * 10.0 ** ((tx_power_dbm - operating_tx_dbm) / 10.0)
        )
        ...
        current_sinr = (
            float(
                np.interp(
                    operating_tx_dbm, tx_power_dbm, full_sinr_db
                )
            )
        ...
        # P_SI is total SI power at the LNA input.  Include the echo sideband
        # power when locating the total-input P1dB limit.
        p1db_total_mw = 10.0 ** (self._float("lna_ip1db_dbm", -20.0) / 10.0)
        echo_total_mw = echo_mw * (1.0 + m2)
        si_total_mw = p_si_mw
        input_noise_mw = self._noise_power_mw()
        total_lna_input_mw = si_total_mw + echo_total_mw + input_noise_mw
        total_lna_input_dbm = 10.0 * np.log10(
            np.maximum(total_lna_input_mw, 1e-300)
        )
        over_compression = np.flatnonzero(
            total_lna_input_mw >= p1db_total_mw
        )
        if len(over_compression):
            idx = int(over_compression[0])
            if idx > 0:
                compression_tx_dbm = float(
                    np.interp(
                        self._float("lna_ip1db_dbm", -20.0),
                        total_lna_input_dbm[idx - 1:idx + 1],
                        tx_power_dbm[idx - 1:idx + 1],
                    )
                )
            else:
                compression_tx_dbm = float(tx_power_dbm[0])
        else:
            compression_tx_dbm = float("nan")
        linear_valid = (
            tx_power_dbm <= compression_tx_dbm
            if np.isfinite(compression_tx_dbm)
            else np.ones_like(tx_power_dbm, dtype=bool)
        )
        return {
            "tx_power_dbm": tx_power_dbm,
            "si_power_dbm": p_si_dbm,
            "full_sinr_db": full_sinr_db,
            "raw_full_sinr_db": np.where(
                np.isfinite(raw_full_sinr),
                10.0 * np.log10(np.maximum(raw_full_sinr, 1e-300)),
                np.nan,
            ),
            "without_si_sinr_db": np.where(
                np.isfinite(without_si_sinr),
                10.0
                * np.log10(np.maximum(without_si_sinr, 1e-300)),
                np.nan,
            ),
            "raw_without_si_sinr_db": np.where(
                np.isfinite(raw_without_si_sinr),
                10.0
                * np.log10(np.maximum(raw_without_si_sinr, 1e-300)),
                np.nan,
            ),
            ...
            "current_tx_dbm": float(operating_tx_dbm),
            "current_si_dbm": float(
                self._tx_dbm_to_si_input_dbm(
                    operating_tx_dbm
                )
            ),
            "current_sinr_db": current_sinr,
            "compression_tx_dbm": float(compression_tx_dbm),
            "linear_valid": np.asarray(linear_valid, dtype=bool),
            "reference_si_dbm": float(10.0 * np.log10(si_ref_mw)),
            "echo_power_dbm": 10.0
            * np.log10(np.maximum(echo_mw, 1e-300)),
            "input_noise_power_dbm": float(
                10.0 * np.log10(max(input_noise_mw, 1e-300))
            ),
            "sensing_residual_ceiling_db": self._sensing_residual_parameters()[0],
            "sensing_quadratic_fraction": self._sensing_residual_parameters()[1],
        }

    def _legacy_draw_tx_power_axis(self, ax, for_save: bool = False) -> None:
        from matplotlib.ticker import (
            AutoMinorLocator,
            FixedLocator,
            MultipleLocator,
        ...
        curves = self._legacy_tx_power_sweep_curves()
        p_tx = np.asarray(curves["tx_power_dbm"], dtype=np.float64)
        full = np.asarray(curves["full_sinr_db"], dtype=np.float64)
        linear_valid = np.asarray(
            curves.get("linear_valid", np.ones_like(p_tx, dtype=bool)),
            dtype=bool,
        )
        full_plot = np.where(linear_valid, full, np.nan)
        linewidth = 2.0 if for_save else 1.6
        with_si_line, = ax.plot(
            p_tx,
            full_plot,
            color="#ff0000",
            linewidth=linewidth,
        ...
        without_si_line, = ax.plot(
            p_tx,
            np.where(
                linear_valid,
                np.asarray(
                    curves["without_si_sinr_db"],
                    dtype=np.float64,
                ),
                np.nan,
            ),
        ...
        compression_dbm = float(curves["compression_tx_dbm"])
        if np.isfinite(compression_dbm):
            ...
            if compression_dbm < float(np.max(p_tx)):
                ax.axvspan(
                    max(compression_dbm, float(np.min(p_tx))),
                    float(np.max(p_tx)),
                    color="#d1d5db",
                    alpha=0.28,
                    linewidth=0.0,
        ...
        current_tx = float(curves["current_tx_dbm"])
        current_sinr = float(curves["current_sinr_db"])
        operating_linear = (
            not np.isfinite(compression_dbm)
            or current_tx <= compression_dbm
        )
        if (
            operating_linear
            and float(np.min(p_tx)) <= current_tx <= float(np.max(p_tx))
        ):
            ax.scatter(
                [current_tx],
                [current_sinr],
        ...
        ax.set_xlim(float(np.min(p_tx)), float(np.max(p_tx)))
        y_min, y_max = self._float_pair("sinr_ylim_db", (0.0, 40.0))
        if y_max > y_min:
        ...
        plot_range = max(
            self._float(
                "ref_range_m",
                self._float("detector_ref_range_m", 1.0),
            ),
            1e-9,
        )
        ax.set_title(f"Sensing SINR at R={plot_range:g} m")
        ax.set_xlabel(r"THz transmit power, $P_t$ (dBm)")
        ax.set_ylabel("Sensing SINR (dB)")
        secondary = ax.secondary_xaxis(
            "top",
            functions=(
                self._tx_dbm_to_si_input_dbm,
                self._si_input_dbm_to_tx_dbm,
            ),
        )
        ...
        if not for_save or bool(self.save_legend_var.get()):
            legend = ax.legend(
                handles=[with_si_line, without_si_line],
                loc="best",
                fontsize=10.5 if for_save else 8.5,
                frameon=True,
            )
        ...
        ax.text(
            ...
            (
                ...
                + f"\ncalibration={self._float('sensing_calibration_db', 0.0):+.1f} dB"
                + (
                    f"\nresidual ceiling="
                    f"{float(curves['sensing_residual_ceiling_db']):g} dB, "
                    f"SI$^2$ fraction="
                    f"{float(curves['sensing_quadratic_fraction']):g}"
                )
            ),
    ...
    def _draw_si_power_axis(self, ax, for_save: bool = False) -> None:
        """Draw the fixed-range closed-form SI sweep from manuscript Eq. (9)."""
        from matplotlib.ticker import AutoMinorLocator, MultipleLocator

        curves = self._si_power_sweep_curves()
        p_si = np.asarray(curves["si_power_dbm"], dtype=np.float64)
        with_si = np.asarray(curves["with_si_db"], dtype=np.float64)
        without_si = np.asarray(curves["without_si_db"], dtype=np.float64)
        ideal_homodyne = np.asarray(
            curves["ideal_homodyne_db"], dtype=np.float64
        )
        linewidth = 2.0 if for_save else 1.6
        linear_min = float(curves["linear_region_min_dbm"])
        linear_max = float(curves["linear_region_max_dbm"])
        if np.isfinite(linear_min) and np.isfinite(linear_max):
            ax.axvspan(
                linear_min,
                linear_max,
                color="#d9f0fa",
        ...
        _comm_threshold_db, sensing_threshold_db = self._required_sinr_db()
        ax.axhline(
            sensing_threshold_db,
            color="#666666",
            linestyle=(0, (2, 2)),
            linewidth=1.0,
        )
        ...
        if (
            not for_save
            and float(np.min(p_si)) <= current_si <= float(np.max(p_si))
        ):
            ax.scatter(
                [current_si],
                [current_sinr],
            ...
            si_off_level = float(without_si[0])
            ideal_current = float(
                np.interp(current_si, p_si, ideal_homodyne)
            )
            gain_x = current_si + 1.2
            ax.annotate(
                "",
                xy=(gain_x, ideal_current),
                xytext=(gain_x, si_off_level),
                arrowprops={
                    "arrowstyle": "<->",
                    "color": "#111111",
                    "linewidth": 0.9,
        ...
        x_min = float(np.min(p_si))
        x_max = float(np.max(p_si))
        ...
        if for_save:
            ...
            self._si_cursor_axes = ax
            self._si_cursor_x = p_si.copy()
            self._si_cursor_y = with_si.copy()
            self._si_cursor_marker, = ax.plot(
                [],
                [],
                linestyle="None",
                marker="o",
            ...
            self._si_cursor_annotation = ax.annotate(
                "",
                xy=(0.0, 0.0),
                xytext=(10, 10),
            ...
            self._si_cursor_annotation.set_visible(False)
        self._style_ieee_axis(ax)
    ...
    def _reprocess_photocurrent_capture(
        ...
        with np.load(path, allow_pickle=True) as loaded:
            primary_channel = (
                str(np.asarray(loaded["rx_primary_channel"]).reshape(-1)[0])
                .strip()
                .upper()
                if "rx_primary_channel" in loaded.files
                else ""
            )
            if "rx__C2__sig" in loaded.files and "rx__C2__fs" in loaded.files:
                c2_signal = np.asarray(loaded["rx__C2__sig"], dtype=np.float64)
                c2_fs = float(np.asarray(loaded["rx__C2__fs"]).reshape(-1)[0])
            elif (
                primary_channel == "C2"
                and "rx_sig" in loaded.files
                and "rx_fs" in loaded.files
            ):
                c2_signal = np.asarray(loaded["rx_sig"], dtype=np.float64)
                c2_fs = float(np.asarray(loaded["rx_fs"]).reshape(-1)[0])
            else:
        ...
        dso.radar_proc_gain_eff_var = self._offline_value_var(26.0)
        ...
        return {
            ...
            "range_profile_contrast_db": float(
                result.get("range_profile_contrast_db", float("nan"))
            ),
            "range_floor_iqr_db": float(
                result.get("range_floor_iqr_db", float("nan"))
            ),
            "stored_c2_snr_db": stored_c2_snr_db,
            "target_peak_m": peak_m,
            "pslr_db": float(result.get("pslr_db", float("nan"))),
            "ideal_processing_gain_db": float(
                result.get("ideal_processing_gain_db", float("nan"))
            ),
            "processing_gain_db": float(
                result.get("processing_gain_db", float("nan"))
            ),
            "pilot_weighted_gain_db": float(
                result.get("pilot_weighted_gain_db", float("nan"))
            ),
            "pre_dsp_target_sinr_db": float(
                result.get("pg_corrected_snr_db", float("nan"))
            ),
            "cfr_coherence": float(
    ...
    def _photocurrent_model_curves(self) -> dict[str, np.ndarray | float]:
        ...
        sensing_on = self._apply_sensing_residual_ceiling(sensing_on_raw)
        sensing_off = self._apply_sensing_residual_ceiling(sensing_off_raw)
        return {
    ...
    def _draw_photocurrent_sinr_figure(
        ...
        for modulation in ("32QAM", "16QAM"):
            ...
            detected = np.isfinite(sensing) & (
                sensing >= self._required_sinr_db()[1]
            )
            if np.any(~detected):
                ax_sens.scatter(
                    x[~detected],
        ...
        _comm_threshold_db, sensing_threshold = self._required_sinr_db()
        ax_sens.axhline(
            sensing_threshold,
            color="#666666",
            linestyle=(0, (2, 2)),
    ...
    def _rmax_vs_effective_rcs(
        self, rcs_dbsm: np.ndarray, rho: float
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """Return the closed-form Fig. 2 range bounds.

        With SI, use the manuscript's long-range approximation
        P_SI >> P_ec and retain only the R^-4 cross-beat.  Without SI, retain
        only the R^-8 echo self-beat.  These theory curves do not use the C2
        detector simulation, practical residual ceiling, or measured points.
        """
        rcs = np.asarray(rcs_dbsm, dtype=np.float64)
        rho_v = float(np.clip(rho, 1e-9, 1.0 - 1e-9))
        context = self._closed_form_theory_context()
        context_rho = max(context["rho"], 1e-30)
        rho_scale = rho_v / context_rho

        _comm_threshold_db, sensing_threshold_db = self._required_sinr_db()
        sensing_threshold = 10.0 ** (sensing_threshold_db / 10.0)
        rcs_sqm = 10.0 ** (rcs / 10.0)
        echo_r4_coefficient = (
            context["echo_per_pt_unit_rcs"]
            * context["pt_mw"]
            * rcs_sqm
        ...
        # Eq. (10): cross-beat only with the SSBI term neglected,
        # gamma=A4/R^4.
        a4 = (
            prefactor
            * si_mw
            * echo_r4_coefficient
            / max(denominator_on, 1e-300)
        )
        r_sens_on = np.maximum(
            a4 / max(sensing_threshold, 1e-300), 0.0
        ) ** 0.25

        # Eq. (11): echo self-beat only, gamma=A8/R^8.
        a8_off = (
            prefactor
            * echo_r4_coefficient ** 2
            / max(noise_mw, 1e-300)
        )
        r_sens_off = np.maximum(
            a8_off / max(sensing_threshold, 1e-300), 0.0
        ) ** 0.125

        comm_threshold_db, _unused = self._required_sinr_db()
        comm_threshold = 10.0 ** (comm_threshold_db / 10.0)
        data_scale = (
            (1.0 - rho_v)
            / max(1.0 - context["ref_rho"], 1e-30)
        )
        tx_scale = (
            context["pt_mw"] / max(context["ref_pt_mw"], 1e-30)
        ) ** 2
        modulation_scale = context["m2"] / max(context["ref_m2"], 1e-30)
        comm_range = context["ref_range_m"] * (
            context["comm_anchor_lin"]
            * data_scale
            * tx_scale
            * modulation_scale
            / max(comm_threshold, 1e-300)
        ) ** 0.25
        r_comm = np.full_like(rcs, float(comm_range), dtype=np.float64)
        r_isac = np.minimum(r_comm, r_sens_on)
        return r_comm, r_sens_on, r_sens_off, r_isac

    def _draw_rcs_range_axis(
        self,
        ax,
        rho: float | None = None,
        for_save: bool = False,
        ...
        rho_v = float(np.clip(self._float("rho", 0.20) if rho is None else rho, 1e-9, 1.0 - 1e-9))
        rcs_min, rcs_max = self._float_pair(
            "rcs_limits_dbsm", (-30.0, 0.0)
        )
        if rcs_max <= rcs_min:
            rcs_max = rcs_min + 1.0
        n_rcs = 201
        rcs_axis = np.linspace(rcs_min, rcs_max, n_rcs)
        r_comm, r_sens_on, r_sens_off, r_joint_on = self._rmax_vs_effective_rcs(rcs_axis, rho_v)

        ax.grid(True, which="major", color="#cbd5e1", linewidth=0.55, alpha=0.75)
        ax.plot(rcs_axis, r_comm, color=blue, linewidth=linewidth,
                label=r"$R_{\max}^{\mathrm{comm}}$")
        ax.plot(rcs_axis, r_sens_on, color=red, linewidth=linewidth,
                label=r"$R_{\max}^{\mathrm{sens}}$ (with SI)")
        if np.any(np.isfinite(r_sens_off)):
            ax.plot(rcs_axis, r_sens_off, color=green, linestyle="--", linewidth=linewidth * 0.9,
                    label=r"$R_{\max}^{\mathrm{sens}}$ (without SI)")
        ax.plot(rcs_axis, r_joint_on, color=black, linewidth=linewidth,
                label=r"$R_{\max}$ (ISAC, with SI)")
        ax.set_xscale("linear")
        ax.set_xlim(rcs_min, rcs_max)
        ax.set_xlabel(r"Effective RCS, $\sigma_{\mathrm{eff}}$ (dBsm)")

        ax.set_yscale("log")
        ax.set_ylim(0.1, 5.0)
        ax.yaxis.set_major_locator(
            FixedLocator([0.1, 0.2, 0.5, 1.0, 2.0, 5.0])
        )
        ...
        if not for_save and hasattr(self, "canvas"):
            self._rcs_cursor_axes = ax
            self._rcs_cursor_series = [
                ("ISAC (with SI)", rcs_axis.copy(), r_joint_on.copy(), black),
                ("Sensing (with SI)", rcs_axis.copy(), r_sens_on.copy(), red),
                (
                    "Sensing (without SI)",
                    rcs_axis.copy(),
                    r_sens_off.copy(),
                ...
                ("Communication", rcs_axis.copy(), r_comm.copy(), blue),
            ]
            self._rcs_cursor_marker, = ax.plot(
                [],
                [],
                linestyle="None",
                marker="o",
            ...
            self._rcs_cursor_annotation = ax.annotate(
                "",
                xy=(0.0, 1.0),
                xytext=(10, 10),
                textcoords="offset points",
            ...
            self._rcs_cursor_annotation.set_visible(False)
        self._style_ieee_axis(ax)

    def _draw_rcs_comparison_figure(self, fig) -> None:
        fig.set_size_inches(5.0, 4.0, forward=True)
        ax = fig.add_subplot(111)
        self._draw_rcs_range_axis(
            ax,
            rho=self._float("rho", 0.20),
            for_save=True,
            show_legend=True,
        )

    def _save_rcs_range_png(self) -> None:
        path_str = filedialog.asksaveasfilename(
            parent=self.parent,
            # title="Save rho Trade-off Figure",
            defaultextension=".png",
            filetypes=[("PNG figure", "*.png")],
            initialfile="isac_range_vs_effective_rcs.png",
        )
        if not path_str:
            return
        try:
            fig = Figure(figsize=(5.0, 4.0), dpi=600)
            self._draw_rcs_comparison_figure(fig)
            fig.tight_layout(pad=0.35)
            fig.savefig(path_str, dpi=600, bbox_inches="tight", facecolor="white")
            self.status_var.set(f"Saved effective-RCS PNG: {Path(path_str).name}")
        except Exception as exc:
            messagebox.showerror("Save effective-RCS PNG", str(exc), parent=self.parent)
    ...
    def _default_radar_measurements(self) -> list[dict[str, float | str]]:
        """Reprocess built-in raw C2 captures with the current DSO metric path."""
        base = APP_DIR / "data" / "captures" / "range_1100mm"
        specs = [
            ("C2 sensing 1100 mm", base / "Data_range_fIF11_fsym15_P-8_fRF280_DFT-s-OFDM_32QAM_Iph7.npz", 1100.0, False, True),
        ]
        points: list[dict[str, float | str]] = []
        for label, path, range_mm, _use_reference, include_radar_snr in specs:
            if not path.exists():
                continue
            try:
                processed = self._reprocess_photocurrent_capture(
                    path,
                    float(range_mm) * 1e-3,
                    sensing_threshold_db=-math.inf,
                )
                snr_db = float(processed.get("sensing_sinr_db", float("nan")))
                pg_db = float(processed.get("processing_gain_db", float("nan")))
                source = str(processed.get("source", "current DSO reprocessing"))
                ...
                points.append({
                    ...
                    "radar_pilot_weighted_gain_db": float(
                        processed.get("pilot_weighted_gain_db", float("nan"))
                    ),
                    "radar_pre_snr_db": float(
                        processed.get("pre_dsp_target_sinr_db", float("nan"))
                    ),
    ...
    def _manual_measurements(self) -> list[dict[str, float | str]]:
        range_evm = self._parse_manual_points(
            "manual_range_evm_points",
            "evm_db",
            "Manual EVM",
        )
        c2_power = self._parse_manual_points(
            "manual_c2_si_on_points",
            "c2_inband_power_dbm",
            "Manual C2 SI-on",
            "on",
        )
        return range_evm + c2_power

    def _campaign_measurements(self) -> list[dict[str, float | str]]:
        points: list[dict[str, float | str]] = []
        records = self.runtime.get("measurement_set_records", [])
        if not isinstance(records, list):
            return points
        for record in records:
            if not isinstance(record, dict) or record.get("condition") != "target_on":
                continue
            try:
                range_m = float(record.get("range_mm", float("nan"))) * 1e-3
            except Exception:
                continue
            ...
            evm_db = metric_value("evm_db")
            sensing_db = metric_value("snr_rad_post_db_c2")
            if not np.isfinite(sensing_db):
                sensing_db = metric_value("snr_rad_db")
            derived = record.get("derived", {})
            try:
                comm_snr_db = float(
                    derived.get("comm_snr_from_rx_noise_db", float("nan"))
                )
            except Exception:
                comm_snr_db = float("nan")
            channels = record.get("channels", {})
            try:
                c2 = channels.get("C2", {})
                c2_power_dbm = float(c2.get("raw_power_dbm", float("nan")))
            except Exception:
                c2_power_dbm = float("nan")
            point: dict[str, float | str] = {
                "name": f"Campaign target-on {range_m * 1e3:.0f} mm",
                "range_m": range_m,
                "source": str(record.get("set_name", "measurement set")),
                "c2_si_state": "on",
            }
            if np.isfinite(evm_db):
                point["evm_db"] = evm_db
            if np.isfinite(comm_snr_db):
                point["snr_comm_db"] = comm_snr_db
            elif np.isfinite(evm_db):
                point["snr_comm_db"] = -evm_db
            if np.isfinite(sensing_db):
                point["snr_sens_db"] = sensing_db
            if np.isfinite(c2_power_dbm):
                point["c2_inband_power_dbm"] = c2_power_dbm
            points.append(point)
        return points

    def _active_measurements(self) -> list[dict[str, float | str]]:
        manual = self._manual_measurements() + self._campaign_measurements()
        manual_c2_ranges: dict[str, list[float]] = {"on": [], "off": []}
        manual_evm_ranges: list[float] = []
        for point in manual:
            ...
            if (
                np.isfinite(rr)
                and np.isfinite(
                    float(point.get("evm_db", float("nan")))
                )
            ):
                manual_evm_ranges.append(rr)
        ...
        for p in self.meas_points:
            ...
            has_evm = (
                np.isfinite(float(p.get("evm_db", float("nan"))))
                or np.isfinite(
                    float(p.get("snr_comm_db", float("nan")))
                )
            )
            same_evm_range_override = any(
                np.isfinite(rr) and abs(rr - manual_rr) <= 5e-4
                for manual_rr in manual_evm_ranges
            )
            retained = dict(p)
            changed = False
            if has_c2 and same_c2_range_override:
                retained["c2_inband_power_dbm"] = float("nan")
                changed = True
            if has_evm and same_evm_range_override:
                retained["evm_db"] = float("nan")
                retained["snr_comm_db"] = float("nan")
                changed = True
            active.append(retained if changed else p)
    ...
    def _c2_target_power_ref_dbm(self) -> float:
        """Phase-averaged target-only C2 power used for sensing SINR."""
        entered = self._finite_param("c2_target_power_ref_dbm")
        if np.isfinite(entered):
            return float(entered)
        return -42.52
    ...
    def _c2_target_power_curve_dbm(self, ranges: np.ndarray, si_state: str) -> np.ndarray:
        ...
        return 10.0 * np.log10(np.maximum(target_mw, 1e-30))

    def _c2_power_curve_dbm(self, ranges: np.ndarray, si_state: str) -> np.ndarray:
        """Return raw IF-band power, including the range-independent background."""
        r = np.maximum(np.asarray(ranges, dtype=np.float64), 1e-12)
        target_dbm = self._c2_target_power_curve_dbm(r, si_state)
        if not np.any(np.isfinite(target_dbm)):
            return np.full_like(r, np.nan, dtype=np.float64)
        target_mw = 10.0 ** (target_dbm / 10.0)
        if str(si_state).strip().lower() == "off":
            # Raw no-SI power approaches the receiver-noise background even
            # though its echo-only component continues to decay as R^-8.
            background_mw = 10.0 ** (self._c2_noise_power_dbm() / 10.0)
            return 10.0 * np.log10(np.maximum(background_mw + target_mw, 1e-30))
        # The measurement points are raw spectrum integrals.  Preserve the
        # range-independent SI/noise background while using the full target
        # curve (R^-4 cross-beat plus R^-8 echo self-beat).
        raw_ref_mw = 10.0 ** (self._c2_power_ref_dbm() / 10.0)
        target_ref_mw = 10.0 ** (self._c2_target_power_ref_dbm() / 10.0)
        background_mw = max(raw_ref_mw - target_ref_mw, 0.0)
        return 10.0 * np.log10(np.maximum(background_mw + target_mw, 1e-30))
    ...
    def _sensing_sinr_from_target_power_dbm(
        ...
        raw_sinr_db = (
            ...
            + self._float("sensing_calibration_db", 0.0)
        )
        raw_sinr = 10.0 ** (np.asarray(raw_sinr_db, dtype=np.float64) / 10.0)
    ...
    def _c2_power_radar_snr_points(self, si_state: str | None = None) -> list[tuple[float, float]]:
        ...
        for point in self._active_measurements():
            ...
            if np.isfinite(rr) and rr > 0.0 and np.isfinite(snr):
                pts.append((rr, snr))
        if state_filter not in ("", "on") or not pts:
            return sorted(pts)

        raw_points = self._c2_power_points("on")
        if len(raw_points) < 2:
            return sorted(pts)
        anchor_r, anchor_snr = min(
            pts,
            key=lambda p: min(abs(p[0] - raw_r) for raw_r, _raw_p in raw_points),
        )
    ...
    def _derived_radar_ref_snr_db(self) -> float:
        return float(
            self._sensing_sinr_from_target_power_dbm(
                self._c2_target_power_ref_dbm(), "on"
            )
        )

    def _sensing_reference_diagnostics(self) -> dict[str, float]:
        """Expose the main model-bias terms at the selected reference range."""
        receiver_prediction = self._derived_radar_ref_snr_db()
        rho = float(np.clip(self._float("rho", 0.20), 1e-12, 1.0))
        spectrum_noise = self._finite_param("c2_spectrum_noise_power_dbm")
        spectrum_prediction = (
            self._c2_target_power_ref_dbm()
            - spectrum_noise
            + float(self._pilot_weighted_processing_gain_db(rho))
            + self._float("sensing_calibration_db", 0.0)
            if np.isfinite(spectrum_noise)
            else float("nan")
        )
        ref_r = max(self._float("ref_range_m", 1.0), 1e-9)
        measured = [
            (
                abs(float(p.get("range_m", float("nan"))) - ref_r),
                float(p.get("snr_sens_db", float("nan"))),
                float(p.get("range_m", float("nan"))),
            )
            for p in self._active_measurements()
            if np.isfinite(float(p.get("range_m", float("nan"))))
            and np.isfinite(float(p.get("snr_sens_db", float("nan"))))
        ]
        measured_prediction = float("nan")
        model_at_measurement = float("nan")
        measured_range_m = float("nan")
        measured_bias = float("nan")
        if measured:
            distance_error, measured_prediction, measured_range_m = min(
                measured, key=lambda item: item[0]
            )
            if distance_error <= max(0.05, 0.15 * ref_r) + 1e-12:
                model_at_measurement = float(
                    np.asarray(
                        self._snr_curve_from_ref(
                            np.asarray([measured_range_m]),
                            receiver_prediction,
                            rho,
                            "sensing",
                        )
                ...
                measured_bias = measured_prediction - model_at_measurement
            else:
                measured_prediction = float("nan")
                measured_range_m = float("nan")
        return {
            "receiver_prediction_db": float(receiver_prediction),
            "spectrum_prediction_db": float(spectrum_prediction),
            "measured_db": float(measured_prediction),
            "measured_range_m": float(measured_range_m),
            "model_at_measurement_db": float(model_at_measurement),
            "measured_minus_model_db": float(measured_bias),
        }
    ...
    def _sync_theory_params_from_cfg(self, cfg: SimConfig) -> None:
        ...
        self.params["target_rcs_mode"].set("direct_effective")
    ...
    def _store_detector_reference_from_sim(self, cfg: SimConfig, data: dict) -> None:
        ...
        values = {
            ...
            "c2_spectrum_noise_power_dbm": c2m.get(
                "noise_power_dbm", float("nan")
            ),
    ...
    def _sync_from_sim(self) -> None:
        try:
            ...
            # it reflected the newly synchronized antenna load.
            self.sim_sweep = None
            self._evm_radar_plot_cache = None
            if isinstance(getattr(self, "symbol_rate_sweep", None), dict):
            ...
            if reference_is_current:
                ...
                if not packaged_reference_loaded:
                    self.params["detector_reference_source"].set("")
            measured_gp_db = self._as_float(
                self.runtime.get("measured_processing_gain_db", float("nan"))
            )
            if np.isfinite(measured_gp_db):
                self.params["radar_proc_gain_db"].set(f"{measured_gp_db:.6g}")
            measured_rcs_dbsm = self._as_float(
                self.runtime.get("estimated_effective_rcs_dbsm", float("nan"))
            )
            if np.isfinite(measured_rcs_dbsm):
                self.params["effective_rcs_dbsm"].set(f"{measured_rcs_dbsm:.6g}")
            suffix = (
    ...
    def _sweep_metric_row(self, data: dict, fixed_noise_dbm: float | None = None, fixed_pg_db: float | None = None) -> dict[str, float]:
        ...
        c2m = data.get("c2_band_metrics", {})
        c2rawm = data.get("c2_raw_band_metrics", {})
        c2_target_power_dbm = float(c2m.get("band_power_dbm", float("nan")))
        ...
        return {
            ...
            "c2_target_power_dbm": c2_target_power_dbm,
    ...
    def _run_distance_sweep(self, ranges: np.ndarray) -> dict[str, np.ndarray] | None:
        ...
        for state in ("on", "off"):
            rows = raw[state]
            for key in (
                "evm_db", "comm_snr_db", "c2_power_dbm", "c2_target_power_dbm",
                "c2_noise_dbm", "pre_snr_db", "post_snr_db", "pg_db",
            ):
        ...
        # The C2-power plot is a like-for-like comparison of raw IF-spectrum
        # integrals. Do not impose a target range law on this total: the
        # range-independent SI-only and noise terms can make it nearly flat.
        # Apply R^-8/R^-4 only to the separated target-only power used by the
        # sensing-SINR calculation.
        if np.isfinite(out["off_c2_target_power_dbm"][ref_idx]):
            off_p_ref = 10.0 ** (float(out["off_c2_target_power_dbm"][ref_idx]) / 10.0)
            off_p = off_p_ref * (law_r / law_ref_r) ** (-8.0)
            out["off_c2_target_power_raw_dbm"] = out["off_c2_target_power_dbm"].copy()
            out["off_c2_target_power_dbm"] = 10.0 * np.log10(np.maximum(off_p, 1e-30))
        if np.isfinite(out["on_c2_target_power_dbm"][ref_idx]):
            on_p_ref = 10.0 ** (float(out["on_c2_target_power_dbm"][ref_idx]) / 10.0)
            off_p_ref = (
                10.0 ** (float(out["off_c2_target_power_dbm"][ref_idx]) / 10.0)
                if np.isfinite(out["off_c2_target_power_dbm"][ref_idx])
                else 0.0
            )
            hom_p_ref = max(on_p_ref - off_p_ref, 0.0)
            off_p = (
                10.0 ** (out["off_c2_target_power_dbm"] / 10.0)
                if np.any(np.isfinite(out["off_c2_target_power_dbm"]))
                else np.zeros_like(law_r)
            )
            hom_p = hom_p_ref * (law_r / law_ref_r) ** (-4.0)
            out["on_c2_target_power_raw_dbm"] = out["on_c2_target_power_dbm"].copy()
            out["on_c2_target_power_dbm"] = 10.0 * np.log10(np.maximum(off_p + hom_p, 1e-30))
        ...
        for state in ("on", "off"):
            ...
            effective_radar_sinr = self._apply_sensing_residual_ceiling(
                raw_radar_sinr
            )
            out[f"{state}_radar_snr_raw_db"] = raw_radar_snr_db
            out[f"{state}_radar_snr_db"] = 10.0 * np.log10(
                np.maximum(effective_radar_sinr, 1e-300)
            )
    ...
    def _run(self, use_cached_sweep: bool = False) -> None:
        try:
            ...
            r_min_raw, r_max_raw = self._float_pair(
                "range_limits_m", (0.5, 2.0)
            )
            r_min = max(r_min_raw, 1e-4)
            r_max = max(r_max_raw, r_min * 1.01)
            plot_x_min = r_min
            plot_x_max = r_max
            n_range = max(3, min(self._int("sweep_points", 9), 101))
            ...
            rho = float(np.clip(self._float("rho", 0.20), 1e-9, 1.0 - 1e-9))
            comm_thr, sens_thr = self._required_sinr_db()
            ...
            if sweep is not None:
                ...
                sim_comm_linear = self._comm_sinr_from_reference(
                    ranges,
                    rho,
                    tx_power_dbm=operating_tx_dbm,
                )
                sim_comm_snr = 10.0 * np.log10(
                    np.maximum(sim_comm_linear, 1e-300)
                )
                if not np.any(np.isfinite(sim_comm_snr)):
                ...
                sim_c2_on_target_power = np.asarray(
                    sweep.get("on_c2_target_power_dbm", np.full_like(ranges, np.nan)),
                    dtype=np.float64,
                )
                sim_c2_off_target_power = np.asarray(
                    sweep.get("off_c2_target_power_dbm", np.full_like(ranges, np.nan)),
                    dtype=np.float64,
                )
                c4, c8 = self._detector_domain_sensing_coefficients(
                    rho,
                ...
                range_safe = np.maximum(
                    np.asarray(ranges, dtype=np.float64), 1e-12
                )
                sensing_on_raw = (
                    np.asarray(c4, dtype=np.float64) / range_safe ** 4
                    + np.asarray(c8, dtype=np.float64) / range_safe ** 8
                )
                si_scale = self._detector_si_power_scale(
                    operating_tx_dbm
                )
                noise_on_mw = float(
                    self._detector_output_noise_mw(si_scale)
                )
                noise_off_mw = float(
                    self._detector_output_noise_mw(0.0)
                )
                ...
                sensing_on_linear = self._apply_sensing_residual_ceiling(
                    sensing_on_raw
                )
                sensing_off_linear = self._apply_sensing_residual_ceiling(
                    sensing_off_raw
                )
                if np.any(np.isfinite(sensing_on_linear)):
                    sim_sens_on_snr = 10.0 * np.log10(
                        np.maximum(sensing_on_linear, 1e-300)
                    )
                    sim_sens_off_snr = 10.0 * np.log10(
                        np.maximum(sensing_off_linear, 1e-300)
                    )
                else:
                ...
                target_on = self._c2_target_power_curve_dbm(curve_ranges, "on")
            ...
            # Reset shared logarithmic axes before clearing; Matplotlib
            # otherwise tries to restore the default [0, 1] limits on a log
            # axis and emits a non-positive-limit warning.
            for old_ax in self.fig.axes:
                try:
            ...
            self._evm_radar_plot_cache = {
                ...
                "c2_off_target_power": sim_c2_off_target_power.copy(),
            ...
            self.status_var.set(
                ...
                f"meas/model@{sensing_diag['measured_range_m']:.3g}m="
                f"{sensing_diag['measured_db']:.2f}/"
                f"{sensing_diag['model_at_measurement_db']:.2f} dB, "
                f"meas-model={sensing_diag['measured_minus_model_db']:+.2f} dB, "
                f"cal={self._float('sensing_calibration_db', 0.0):+.2f} dB, "
...
class UnifiedApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("ISAC Unified GUI v2 (TX + DSO + Simulation)")
        ...
        self.dso_panel.tx_source = self.tx_sim_panel
        self.dso_panel.simulation_source = self.photonic_sim_panel
        ...
        self.dso_panel.system_model_source = self.system_model_panel
    ...
    def _sync_dso_from_awg_panel(self, source: str = "AWG panel") -> None:
        try:
            ...
            if hasattr(self.photonic_sim_panel, "params"):
                gp_var = self.photonic_sim_panel.params.get("radar_proc_gain_eff_db")
                if gp_var is not None:
                    self.dso_panel.radar_proc_gain_eff_var.set(gp_var.get())
